...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY : area,iplg,indel,indnd,nne,nnp,np,npa,rkind
  USE elfe_msgp, ONLY : comm,exchange_p2d,ierr,itype,myrank,nproc
  USE elfe_msgp, ONLY : parallel_barrier 
...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY: dldxy,idry,nea,elnode,npa,rkind,xnd,ynd
  USE elfe_msgp, ONLY: comm,exchange_p2d,ierr,itype,myrank,nproc,    &
                       parallel_abort

  USE sed2d_mod, ONLY: idsed2d,slope_cr,vc_area
...
!- User-defined parameters ------------------------------------------
  ...
  REAL(rkind), PARAMETER :: wetslope = 0.2d0
  REAL(rkind), PARAMETER :: dryslope = 1.d0
...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY : indel,isidenode,nea,nne,elnode,np,npa,nsa,rkind
  USE elfe_msgp, ONLY : exchange_e2d,exchange_p2d,exchange_s2d,      &
                        parallel_abort
...
!--------------------------------------------------------------------
  ...
  IF(ndim == npa) THEN !Node-element-node interpolation
    ...
    DO i = 1,nea
       DO j = 1,3
          inode = elnode(j,i)
          tmp_e(i) = tmp_e(i)+var1(inode)/3.D0
       ENDDO
    ENDDO
  ...
  ELSEIF(ndim == nea) THEN !Element-node-element interpolation
    ...
    DO i = 1,nea
       DO j = 1,3
          inode = elnode(j,i)
          var2(i) = var2(i)+tmp_n(inode)/3.d0
       ENDDO
    ENDDO
