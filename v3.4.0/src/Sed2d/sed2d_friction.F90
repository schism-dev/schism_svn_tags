...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY : grav,rho0,rhosed,rkind
...
!--------------------------------------------------------------------
  theta_cr = (0.3d0/(1.d0+1.2d0*D_star))+0.055d0*(1.d0-DEXP(-0.02d0* &
             D_star))
...
!--------------------------------------------------------------------
  IF((d50.GE.0.00005d0) .AND. (d50.LE.0.0005d0)) THEN
    Ucr_c = 0.19d0*d50**0.1d0*DLOG10(4.d0*h/d90)
    Ucr_w = (0.24d0*((s-1.d0)*grav)**0.66d0)*(d50**0.33d0)*          &
            (tp**0.33d0)
  ELSEIF((d50.GT.0.0005d0) .AND. (d50.LE.0.002d0)) then
    Ucr_c = 8.5d0*d50**0.6d0*DLOG10(4.d0*h/d90)
    Ucr_w = (0.95d0*((s-1.d0)*grav)**0.57d0)*(d50**0.43d0)*          &
            (tp**0.14d0)
  ELSE
...
! History:
!--------------------------------------------------------------------
  USE elfe_glbl, ONLY : grav,rho0,rkind
  USE elfe_msgp, ONLY : parallel_abort
...
!--------------------------------------------------------------------
  unorm = DSQRT(u*u+v*v)
  Uwc = DSQRT(uorb*uorb+unorm*unorm)
...
! History:
!--------------------------------------------------------------------
  USE elfe_glbl, ONLY : grav,pi,rho0,rhosed,rkind
  USE elfe_msgp, ONLY : parallel_abort
  USE sed2d_mod, ONLY : s
