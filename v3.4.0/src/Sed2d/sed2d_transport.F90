...
!- Transport rate
  ...
  aux = DSQRT(unorm**2.d0+(0.018d0/Cd)*urms**2.d0)
  IF(((aux-U_cr).GE.0.d0)) THEN
    cff = Asb*unorm*(aux-U_cr)**2.4d0
    qb(1) = alpha_s*cff*DCOS(udir)-alpha_n*alpha_s*cff*DSIN(udir)
    qb(2) = alpha_s*cff*DSIN(udir)-alpha_n*alpha_s*cff*DCOS(udir)
  ENDIF
...
!- Arguments --------------------------------------------------------
  REAL(rkind), INTENT(IN) :: Cd,D_star,d50,u,U_cr,urms,v
...
!- Transport rate
  IF(((aux-U_cr).GE.0.d0)) THEN
    qs(1) = (Ass*unorm*(aux-U_cr)**2.4d0)*DCOS(udir)
    qs(2) = (Ass*unorm*(aux-U_cr)**2.4d0)*DSIN(udir)
  ENDIF
...
!- Transport rate
  qb = 0.d0
  unorm = MIN(DSQRT(u*u+v*v),1.8d0) ! Range of validity for the formula
!  unorm = DSQRT(u*u+v*v)
  udir = DATAN2(v,u)
  ...
  ELSE
    ...
    Me = (ueff-Ucr)/DSQRT((s-1.d0)*grav*d50)
...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY: grav,pi,rho0,rkind
  USE elfe_msgp, ONLY : parallel_abort
...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY : pi,rho0,rkind
  USE elfe_msgp, ONLY : parallel_abort
...
!--------------------------------------------------------------------
  ...
  unorm = DSQRT(u*u+v*v)
  udir = DATAN2(v,u)
  lslope = dpdxy(1)*DCOS(udir)+dpdxy(2)*DSIN(udir) !longitudinal slope
  tslope = dpdxy(2)*DCOS(udir)-dpdxy(1)*DSIN(udir) !transverse slope

  IF(islope == 2) THEN
    sed_angle = DTAN(30.d0*pi/180.d0)
    cff = MIN(ABS(lslope),0.9d0*sed_angle)*DSIGN(1.d0,lslope)
    alpha_s = 1.d0+1.d0*((sed_angle/(DCOS(DATAN(cff))*               &
              (sed_angle-cff)))-1.d0)
    tau = rho0*Cd*unorm**2.d0
    IF(tau == 0.d0) THEN
       alpha_n = 0.d0
    ELSE
       alpha_n = 1.5d0*DSQRT(tau_cr/tau)*tslope
    ENDIF
  ELSE
      alpha_n = 0.d0
      alpha_s = 1.d0
