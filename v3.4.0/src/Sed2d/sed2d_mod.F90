...
!--------------------------------------------------------------------
  ...
  REAL(rkind), ALLOCATABLE :: bc_val(:),   & !Bnd nodes num. flag (-)
                              ...
                              d50(:),      & !Median grain size (m); d50(npa) if nb_class=1; otherwise d50(nb_class)
                              d90(:),      & !D90 grain size (m); for dimension see d50()
                              d50moy(:,:), & !Mean D50 (m) for each sediment layer; d50moy(npa,nb_layer)
                              d90moy(:,:), & !Mean D90 (m) for each sediment layer; d90moy(npa,nb_layer)
                              ...
                              mcoefd(:,:), & !Matrix coef for JCG; mcoefd(0:(mnei+1),np)
                              qav(:,:),    & !Averaged qtot (m3/m/s); qav(npa,2)
                              ...
                              qsum_e(:,:), & !Qtot sum (m3/m/s); qsum_e(nea,2)
