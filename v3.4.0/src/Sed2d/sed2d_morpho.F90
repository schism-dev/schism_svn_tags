...
!
! History:
! 02/2013 - G.Dodet: - Removed arguments shared in elfe_glbl; 
...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY : area,dt,dp,eta2,idry,idry_e,indel,iplg,isdel,     &
                        isidenode,elside,nsa,moitn0,mxitn0,nea,elnode,nne,np,&
                        npa,rkind,rtol0,xctr,xnd,yctr,ynd
  USE elfe_msgp, ONLY : exchange_e2d,exchange_p2d,parallel_abort
...
!YJZ: qtot_e not used; also consider removing imeth=2 altogether, and qtot_s
                        imeth,mcoefd,poro,qdt_e,qtot_e,qtot_s
  USE sed2d_mod, ONLY : nb_class
...
!--------------------------------------------------------------------
  ...
  DO i = 1,nea
     htot = 0.d0
     DO j = 1,3
        htot = htot+(eta2(elnode(j,i))+dp(elnode(j,i)))/3.d0
     ENDDO !j
...
!- Solve Exner equation ---------------------------------------------
  bed_delta = 0.d0 
  CALL solve_jcg(it,moitn0,mxitn0,rtol0,mcoefd,bed_delta,tmp,bc_val, &
                 bc_flag)
