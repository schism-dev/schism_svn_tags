...
!--------------------------------------------------------------------
  ...
  USE elfe_glbl, ONLY: area,dp,iegl,indel,iond_global,ipgl,isbnd,      &
                       ne_global,nea,elnode,nne,nond_global,nope_global, &
                       np_global,mnei,np,npa,nsa,rho0,rhosed,rkind,  &
                       xnd,ynd
  USE elfe_msgp, ONLY: exchange_p2d,myrank,parallel_abort
  USE sed2d_mod, ONLY: bc_flag,bc_val,bed_del_e,bed_delta,cflsed,    &
                       dpdxy,dpdxy_e,dpdxy_s,Cdsed,d50,d90,          &
                       idsed2d,ifilt,ipre_filt,ipre_flag,mcoefd,qav, &
                       qb,qb_e,qb_s,qdt_e,qs,qs_e,qs_s,qtot,qtot_e,  &
                       qtot_s,s,qsum_e,slope_cr,vc_area,z0_e,        &
                       z0cr_e,z0mr_e,z0sw_e,z0wr_e
  USE sed2d_mod, ONLY: u2_tmp,v2_tmp
  USE sed2d_mod, ONLY: d50moy,d90moy,F_class,h2,h_inf,nb_class,      &
                       nb_layer
  USE sed2d_filter
...
!--------------------------------------------------------------------
  IF(myrank == 0) THEN
    OPEN(idsed2d,FILE='sed2d.out',STATUS='REPLACE')
    WRITE(idsed2d,*)'***********************************************'
...
!- Allocate arrays for multi-class multi-layer ----------------------
  ALLOCATE(d50moy(npa,nb_layer),d90moy(npa,nb_layer),h2(npa))

!- Initialize variables ---------------------------------------------
  Cdsed = 0.0025 !for first step in selfe_step
  bed_del_e = 0.d0
  cflsed = 0.d0
  qav = 0.d0
  qb = 0.d0
  ...
  IF (nb_class.GT.1) THEN
    DO i=1,nb_layer
      d50moy(:,i) = d50(1)**F_class(:,1,i)
      d90moy(:,i) = d90(1)**F_class(:,1,i)
      DO j=2,nb_class
        d50moy(:,i) = d50moy(:,i) * d50(j)**F_class(:,j,i)
        d90moy(:,i) = d90moy(:,i) * d90(j)**F_class(:,j,i)
      ENDDO
    ENDDO !nb_layer
  ENDIF !nb_class

  IF (nb_layer.GT.1) THEN
    h2(:) = h_inf
  ENDIF

!- Read input files -------------------------------------------------
  !Up to now, d50(:) are const. read in from sed2d.in
  IF(d50(1).LT.0.d0)THEN
    INQUIRE(FILE='d50.gr3',EXIST=lexist)
    IF(lexist) THEN
      OPEN(fileid,FILE='d50.gr3',STATUS='OLD')
...
!- Read file with critical slopes -----------------------------------
  IF(ifilt >= 2 .OR. ipre_filt == 2) THEN
    ...
    IF(lexist) THEN
      OPEN(fileid,FILE='slope_cr.gr3',STATUS='OLD')
