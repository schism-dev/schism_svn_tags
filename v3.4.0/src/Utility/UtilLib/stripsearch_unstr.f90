...
!            xbg,ybg,nmbg - x,y, and connectivity table for unstr. grid 
...
!            xybin(1:nbin+1) - x or y coord. of each bin line
      subroutine stripsearch_unstr(is_xy,nbin,mne_bin,nebg,npbg,xbg,ybg,nmbg, &
     &ne_bin,ie_bin,xybin,binwid)
      implicit real(8)(a-h,o-z)

      integer, intent(in) :: is_xy,nbin,mne_bin,nebg,npbg,nmbg(nebg,3)
...
!      endif
      ...
      do i=1,nebg
        if(is_xy==1) then
          bmin=minval(xbg(nmbg(i,:))); bmax=maxval(xbg(nmbg(i,:)))
        else
          bmin=minval(ybg(nmbg(i,:))); bmax=maxval(ybg(nmbg(i,:)))
        endif
