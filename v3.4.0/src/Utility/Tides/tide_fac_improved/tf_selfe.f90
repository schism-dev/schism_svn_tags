...
!-------------------------------------------------------------------------------
  ...
  do nc=1,ntip
    ...
    do ncst=1,37
      ...
      if(trim(tname(nc)).eq.trim(ctmp))then
        ncon(nc)=ncst
      endif
    enddo
    if(ncon(nc).eq.0.and.(trim(tname(nc)).ne.'Z0'))then
      write(*,"('no match earth tidal potential constitute for: ',a5,'(',i3,'th of ',i3,' )')")trim(tname(nc)),nc,ntip
      stop
    else
      write(*,"('find match earth tidal potential constitute for: ',a5,'(',i3,'th of ',i3,' )')")trim(tname(nc)),nc,ntip
    endif
  enddo
  do nc=1,nbfr
    ...
    do ncst=1,37
      ctmp=cname(ncst)
      call upper_case(ctmp)
      if(trim(fname(nc)).eq.trim(ctmp))then
        ncon(nc+ntip)=ncst
    endif
    enddo
    if(ncon(nc+ntip).eq.0.and.(trim(fname(nc)).ne.'Z0'))then
      write(*,"('no match constitute for: ',a5,'(',i3,'th of ',i3,' )')")trim(fname(nc)),nc,nbfr
      stop
    else
      if(trim(fname(nc)).ne.'Z0')then
        write(*,"('find match constitute ',a5,' ( #',i3,' ) for: ',a5,'(',i3,'th of ',i3,' )')")&
