...
!  ifort -cpp -O2 -assume byterecl -o combine_output10.WW ../UtilLib/argparse.F90 combine_output10.f90 ../UtilLib/schism_geometry.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
...
30  FORMAT('SCHISM model output version ',A48)
    ...
    iret = nf90_put_att(ncid, NF90_GLOBAL, 'history','created by combine_output10')
...
!              if(i23d(m)/=7) outb(1:2,1:kbs(isd)-1,isd)=NF90_FILL_FLOAT
        ...
        if(m==2) then !idry_e
          idry_e=nint(outb(1,1,1:ne_global))
          idry=1 !init as dry
          idry_s=1
          do i=1,ne_global
            do j=1,i34(i)
...
!
        ...
        if(i23d(m)>0.and.mod(i23d(m)-1,3)/=0) then !3D
          if(i23d(m)<=3) then !node
            do i=1,np_global
              outb(1:2,1:kbp00(i)-1,i)=NF90_FILL_FLOAT
              if(idry(i)==1.and.iwetdry/=0) outb(1:2,:,i)=NF90_FILL_FLOAT
            enddo !i
          else if(i23d(m)<=6) then
            do i=1,ne_global
              outb(1:2,1:kbe(i)-1,i)=NF90_FILL_FLOAT
              if(idry_e(i)==1.and.iwetdry/=0) outb(1:2,:,i)=NF90_FILL_FLOAT
            enddo !i
          else
            do i=1,ns_global
              outb(1:2,1:kbs(i)-1,i)=NF90_FILL_FLOAT
              if(idry_s(i)==1.and.iwetdry/=0) outb(1:2,:,i)=NF90_FILL_FLOAT
            enddo !i
          endif !i23d
        endif !3D
