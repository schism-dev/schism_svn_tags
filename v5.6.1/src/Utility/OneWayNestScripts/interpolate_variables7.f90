...
!          (6) nc outputs from large-domain run (must have elev or temp or salt or hvel depending on ifile)
!       Outputs: 
...
!      integer(kind=8) :: irec,long_rec
     ...
     &count_2d(2),count_3d(3),count_4d(4),var1d_dims(1),var4d_dims(4)
      real*8 :: aa1(1)
...
!...  Header
      ...
      !Returned vars: ne,np,ns,nrec,start_time,[x y dp kbp00](np),
...
!     Read in vgrid.fg 
     ...
     &zfg(npfg,nvrt_fg),kbpfg(npfg),tfg(npfg,nvrt_fg),sfg(npfg,nvrt_fg),cs_fg(nvrt_fg),outvar(2,nvrt,np))
...
!                if(ifile==2) then !ST
!                  long_rec=irec+icum(nd,k,1)
!                  read(63,rec=long_rec) tnd(nd,k)
!                  read(65,rec=long_rec) snd(nd,k)
!                else !uv
!                  long_rec=irec+icum(nd,k,1)
!                  read(63,rec=long_rec) tnd(nd,k) !use for u
!                  long_rec=irec+icum(nd,k,2)
!                  read(63,rec=long_rec) snd(nd,k)
!                endif
!              enddo !k
!            enddo !j
!          enddo !i
