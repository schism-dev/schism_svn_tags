...
!-------------------------------------------------------------------------------
  use cosine_mod, only : ispm,time_cosine,SPM
  use schism_glbl, only : errmsg,rkind,nvrt,ne_global,nea,ipgl,iegl,ihot
  use schism_msgp, only: myrank, parallel_abort
  implicit none
  real(rkind),intent(in) :: time

  !local variables
  integer :: i,j,k,ie,iegb
  real(rkind) :: rtmp,tSPM(ne_global)
