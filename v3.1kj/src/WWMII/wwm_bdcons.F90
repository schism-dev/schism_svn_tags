...
!
        ...
        DO IE = 1, MNE
          NI = INE(:,IE)
          IF (ABS(SUM(IOBP(NI))) .EQ. 2) THEN ! This element has a boundary edge ...
            DO ID = 1, MDC
              IF ( SUM(IOBPD(ID,NI)) .EQ. 1) THEN ! At least two nodes of this element have the IOBPD = 0  
                IOBED(ID,IE) = 0
              ELSE
                IOBED(ID,IE) = 1 
              END IF 
...
! end modification by MDS
           ...
           END IF
           IF ( IFSTAT /= 0 ) CALL MESGERR('error in the bnd file', 'PREPARE')
         END DO
         REWIND(BND%FHNDL)
!
...
!
         IF (LBCWA) THEN ! Parametric Wave Boundary is prescribed 
           ...
           IF (LINHOM) THEN ! Inhomogenous in space 
             IF (LBCSE) THEN
               SPPARM = 0.
               WBAC   = 0.
               IF (TRIM(BOUNDFORMAT) .EQ. 'WWM') THEN 
                 CALL READWAVEPARWWM
               ELSE IF (TRIM(BOUNDFORMAT) .EQ. 'FVCOM') THEN
                 CALL READWAVEPARFVCOM
               ELSE IF (TRIM(BOUNDFORMAT) .EQ. 'WW3') THEN
                 CALL READ_NETCDF_WW3(IFILE,IT)
                 CALL INTER_STRUCT_BOUNDARY(NDX_BND,NDY_BND,DX_BND,DY_BND,OFFSET_X_BND,OFFSET_Y_BND,SPPARM) 
                 CALL INTER_STRUCT_DOMAIN(NDX_BND,NDY_BND,DX_BND,DY_BND,OFFSET_X_BND,OFFSET_Y_BND,TMPPAR) ! Interpolates the whole waves of the whole mesh!
               END IF
               DO IP = 1, IWBMNP
         ...
         ELSE IF (LBCSP) THEN ! Spectrum is prescribed
           IF (LINHOM) THEN ! The boundary conditions is not homogenous!
             IF (TRIM(SPECTYPE) .EQ. 'WWM') THEN
               CALL READSPEC2D(LFIRSTREAD)
             ELSE IF (TRIM(SPECTYPE) .EQ. 'WW3') THEN
...
!     SPPARM(8): peak enhancement factor for the JONSWAP spectra 3.3
      ...
      EFTOT = 0.
      ACLOC = 0.
!AR: Return in case of zero waves or zero period
      IF (SPPAR(1) .LT. SMALL .OR. SPPAR(2) .LT. SMALL) THEN
        ACLOC = 0.
        RETURN
      END IF
...
!       Mean period:
        IF ( ABS(AM1).LT. THR ) THEN
           MPER = PI2 * AM0 / AM1
        ELSE
          WRITE (DBG%FHNDL,*)' first moment is zero'
          WRITE (DBG%FHNDL,*) 'AMi', AM0, AM1
          STOP 'specparam'
        END IF
...
!
!        Interpolate Energy density in Frequency Space, if LCUBIC than Cubic Spline Interpolation is used
!
         ACLOC = 0.
...
!
         ...
         DO IS = 1, MSC
           IF ( SPSIG(IS) .GT. INSPF(WBMSC) ) SPCDIR(IS) = SPDIR(IS-1)
         END DO
!
         DO IS = 1, MSC
           ...
           IF (LPARMDIR) THEN
             ...
             DO IS = 1, MSC
               ...
               DO ID = 1, MDC
                 ...
                 IF (DDACOS .GT. 0.) THEN
                   CDIR1 = CTOT1 * MAX (DDACOS**MS1, SMALL)
                 ELSE
                   CDIR1 = 0. 
                 ENDIF
