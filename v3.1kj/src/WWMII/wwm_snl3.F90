...
!         WRITE (DBG%FHDL,*) 'CHECK RESOLUTION', IRES, FACSCL, FACRES, SIGLOW
!      END IF
...
!AR: to do replace the triad crab of swan by maartens stuff ...
      ISMAX = MAX( 1, MIN( MSC, MAX ( ISMAX , ISP1 ) ) ) ! added fix the bug described below ...
!      ISMAX = MAX ( ISMAX , ISP1 ) 

      IF ( URSELL .GE. PTRIAD(5) .AND. DEP(IP) .GT. DMINTRIAD) THEN
           ...
           DO ID = 1, MDC
             ...
             IF (ABS(STRI) .GT. SMALL) THEN
               IF (ICOMP >= 2) THEN
                 ...
                 ELSE
                   IMATDA(IS,ID) = IMATDA(IS,ID) - STRI / MAX(10E-5,(ACLOC(IS,ID)*SIGPI))
                 END IF
               ELSE
                 IMATRA(IS,ID) = IMATRA(IS,ID) + STRI / SIGPI
