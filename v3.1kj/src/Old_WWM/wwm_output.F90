...
!
         ...
         INTEGER            :: IP, ID, I 
#ifdef SELFE
         REAL               :: OUTT_GLOBAL(NP_GLOBAL,NVARS)
         REAL               :: OUTT(NP_GLOBAL,NVARS)
         INTEGER :: nwild(NP_GLOBAL),nwild_gb(NP_GLOBAL)
#else
         REAL               :: OUTT(MNP,NVARS)
#endif
         REAL               :: ABSWIND(MNP), WLM, KLM
         REAL               :: ACLOC(MSC,MDC), OUTPAR(NVARS)
         CHARACTER(LEN=15)  :: CTIME
         ...
         DO IP = 1, MNP 
            ...
            nwild(iplg(IP))  = 1
         END DO
         call mpi_reduce(nwild,nwild_gb,NP_GLOBAL,itype,MPI_SUM,0,comm,ierr)
         call mpi_reduce(OUTT,OUTT_GLOBAL,NP_GLOBAL*NVARS,MPI_REAL4,MPI_SUM,0,comm,ierr)
         if(myrank==0) then
           do IP=1,NP_GLOBAL
         ...
         IF (myrank == 0) THEN
           ...
           END IF
           ABSWIND = (WINDXY(:,1)**2. + WINDXY(:,2)**2.)**0.5
           DO I = 1, 1 
         ...
         END IF  
#else
!$OMP PARALLEL DO DEFAULT(NONE) SHARED(AC2,FRHIG,WINDXY,MSC,CFLCXY,MNP,DEP,DMIN,OUTT,OUT,LCFL,IOBP,TIME) PRIVATE(IP,OUTPAR,WLM,KLM,ACLOC,ABSWIND)
         DO IP = 1, MNP
            IF (DEP(IP) .GT. DMIN) THEN
              ACLOC(:,:) = AC2(IP,:,:)
              CALL INTPAR(IP, MSC, ACLOC, OUTPAR)
...
#ENDIF SELFE
         ...
         DO I = 1, IOUTS ! Loop over stations ...
           ...
           ELSE
             ...
             WRITE(OUTPARM%FHNDL, TITLEFORMAT) 'TIME', VARNAMES
           END IF 
...
!**********************************************************************
      SUBROUTINE OUTPUT_TEST()
         ...
         IMPLICIT NONE
         INTEGER :: IP
         REAL :: OUTT(MNP,NVARS), ACLOC(MSC,MDC), OUTPAR(NVARS), SIG_MAX
         CHARACTER(LEN=15) :: CTIME
