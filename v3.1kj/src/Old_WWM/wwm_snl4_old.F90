...
!*                                                                    *
!**********************************************************************
      SUBROUTINE SNL41(IP,KMESPC, KP, ACLOC, IMATRA, IMATDA)
         USE DATAPOOL
         IMPLICIT NONE

         INTEGER, INTENT(IN) :: IP
         REAL,    INTENT(IN) :: KMESPC, KP
         ...
         REAL                :: JACOBI, SIGPI, PI3
         REAL                :: FACHFR, PWTAIL
         REAL                :: LAMBDA
         REAL                :: E00, EP1, EM1, EP2, EM2
         REAL                :: SA1A, SA1B, SA2A, SA2B, SFNL(MSC,MDC), DSNL(MSC,MDC)
...
!
         ...
         CONS   = SNLC1 * (1.0 + SNLCS1/AUX*(1.0-SNLCS2*AUX)*EXP(AUX2))
...
!
     ...
     &        + AWG8 * ( SA1(I-ISM ,J+IDM ) + SA2(I-ISM ,J-IDM ) )
              ...
              DSNL(I,ID) =   -2.0 * ( DA1C(I,J) + DA2C(I,J) )        &
     ...
     &        + SWG8 * ( DA1M(I-ISM ,J+IDM ) + DA2M(I-ISM ,J-IDM ) )
              ...
              IF (ICOMP >= 2) THEN
                ...
                IMATDA(I,ID) = IMATDA(I,ID) - DSNL(I,ID) / PI3
              ELSE
                IMATRA(I,ID) = IMATRA(I,ID) + SFNL(I,ID) / SIGPI
                IMATDA(I,ID) = IMATDA(I,ID) + DSNL(I,ID) / PI3
              END IF
...
!*                                                                    *
!**********************************************************************
      SUBROUTINE SNL42(IP, KMESPC , KP, ACLOC, IMATRA, IMATDA)
         USE DATAPOOL
         IMPLICIT NONE
         ...
         REAL                :: PWTAIL, FACHFR
         REAL                :: LAMBDA
         REAL                :: SNLC1, SNLCS1, SNLCS2, SNLCS3
         REAL                :: CONS, FACTOR
         ...
         INTEGER             :: IS, ID, ID0
         REAL                :: AWG1, AWG2, AWG3, AWG4, AWG5, AWG6, AWG7, AWG8
         REAL                :: E00, EP1, EM1, EP2, EM2, SIGPI2
         REAL                :: SA11, SA22, AUX, AUX2
         REAL                :: SA1A, SA1B, SA2A, SA2B, SFNL(MSC,MDC)

         REAL                :: UE(MSC4MI:MSC4MA, MDC4MI:MDC4MA)

! deleted         REAL, ALLOCATABLE :: UE(:,:)
         ...
         DO IS = 1, MSC
           ...
           DO ID = 1, MDC
             IF (ICOMP >= 2) THEN
               ...
               ELSE
                 IMATDA(IS,ID) = IMATDA(IS,ID) - SFNL(IS,ID) / MAX(SMALL,ACLOC(IS,ID)*SIGPI2)
               END IF
             ELSE
               IMATRA(IS,ID) = IMATRA(IS,ID) + SFNL(IS,ID) / SIGPI2 
               IMATDA(IS,ID) = IMATDA(IS,ID) + SFNL(IS,ID) / MAX(SMALL,ACLOC(IS,ID)*SIGPI2)
...
!*                                                                    *
!**********************************************************************
      SUBROUTINE SNL41S(IP,KMESPC, KP, ACLOC, IMATRA, IMATDA)
         USE DATAPOOL
         IMPLICIT NONE
     ...
     &        + AWG8 * ( SA1(I-ISM ,J+IDM ) + SA2(I-ISM ,J-IDM ) )
             ...
             IF (ICOMP .EQ. 2) THEN
               IF (SFNL(I,ID).GT.0.) THEN ! Patankar rule's ...                                      
                 IMATRA(I,ID) = IMATRA(I,ID) + SFNL(I,ID) / SIGPI
               ELSE
                 IMATDA(I,ID) = IMATDA(I,ID) - SFNL(I,ID) / MAX(SMALL,ACLOC(I,ID)*SIGPI)
               END IF
             ELSE
               IMATRA(I,ID) = IMATRA(I,ID) + SFNL(I,ID) / SIGPI
               IMATDA(I,ID) = IMATDA(I,ID) + SFNL(I,ID) / MAX(SMALL,ACLOC(I,ID)*SIGPI)
