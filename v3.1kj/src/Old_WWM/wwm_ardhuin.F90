...
!
      ...
      REAL                    :: WWNMEANP, WWNMEANPTAIL, WNMEANP, WNMEANPTAIL
      REAL                    :: FTE, FTF
      REAL                    :: STXFTF, STXFTWN
      ...
      REAL                    :: SSDSC1, SSDSC2, SSDSC3, SSDSC4, SSDSC5, SSDSC6
      REAL                    :: SSDSBR, SSDSBRF1, SSDSBRF2, SSDSBR2
      REAL                    :: SSDSP
      INTEGER                 :: SSDSISO, SSDSBRFDF
      REAL                    :: SSDSBM(0:4)
      REAL                    :: ZZ0MAX
      LOGICAL                 :: LFIRSTSOURCE = .TRUE.
      REAL,ALLOCATABLE        :: TAUWX(:), TAUWY(:), CD(:), Z0(:), USTDIR(:)

!/
      CONTAINS
!/ ------------------------------------------------------------------- /
      SUBROUTINE W3SPR4 (A, CG, WN, EMEAN, FMEAN, WNMEAN,     &
...
!/
      ...
      REAL, INTENT(IN)        :: CG(NK), K(NK),Z0,U, CD
...
! With MIN and MAX the bug should disappear.... but where did it come from?
        ...
        CONST2=DDEN2(IS)/CG(IK) &        !Jacobian to get energy in band
              *G9/(SIG(IK)/K(IK)*DRAT) ! coefficient to get momentum
          ...
          IF ((SSWELLF(1).NE.0.AND.DSTAB(ISTAB,IS).LT.1E-7*SIG2(IS)) &
            ...
            IF (SSWELLF(4).GT.0) THEN 
                 ...
                 ELSE
                   DSTAB(ISTAB,IS) = DSTAB(ISTAB,IS) &
                                      -DRAT*MAX(                      &
                          2*SSWELLF(5)*K(IK)*SQRT(2*NU_AIR*SIG2(IS)), &
                                 SSWELLF(1)*                          &
...
!/ ------------------------------------------------------------------- /
      SUBROUTINE W3SDS4 (A, K, CG, USTAR, USDIR, DEPTH, S, D)
!/
!/
!/                  +-----------------------------------+
!/                  | WAVEWATCH III           NOAA/NCEP |
...
!/S      INTEGER, SAVE           :: IENT = 0
      INTEGER                 :: IK, ITH, I_INT, IK2, ITH2, IKIND, L,& 
                                 IKHS, IKD, SDSNTH   
      INTEGER                 :: NSMOOTH(NK)
      REAL                    :: FACTOR, COSWIND, ASUM
      REAL                    :: ALFAMEAN, KB
      REAL                    :: FACTURB, DTURB, DCUMULATIVE, BREAKFRACTION
      REAL                    :: RENEWALFREQ, EPSR
      REAL                    :: NTIMES(NK), S1(NK), ATMP(NTH), E1(NK)
      REAL                    :: GAM, XT, M1
      REAL                    ::  DK(NK), HS(NK), KBAR(NK), DCK(NK), QB(NK)
      REAL                    :: EFDF(NK)     ! Energy integrated over a spectral band
      REAL                    :: Q1(NK) 
      INTEGER                 :: IKSUP(NK)
      REAL                    :: BSIGBAJ, SATURATION, SATURATION2, FACSAT, DKHS
...
!
            DCK(IKL)=((KBAR(IKL)**(-2.5))*(KBAR(IKL)/(2*PI)))*DCKI(IKHS,IKD)    ! DCKI is tabulated in INSIN4
          ELSE   
            DCK(IKL)=0.
            END IF  
          END DO
                ...
                IF (BTH0(IK2).GT.SSDSBR) THEN
                  DO ITH2=1,NTH
                    ...
                    RENEWALFREQ=RENEWALFREQ+ &
                         CUMULW(IK,ITH,IK2,ITH2)*(MAX(SQRT(BTH(IS2))-EPSR,0.))**2
                    END DO
                  END IF
                END DO
              END IF
