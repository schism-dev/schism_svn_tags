...
!*                                                                    *
!**********************************************************************
      SUBROUTINE COMPUTE_DIRECTION_QUICKEST_A2(IP,IMATRA,IMATDA)
         USE DATAPOOL
         IMPLICIT NONE
         ...
         DO IS = 1, MSC
           ACQ( 1:MDC) = AC2(IP,IS,:)
           CADS(1:MDC) = CAD(IS,:)
           CFLCAD = MAXVAL(ABS(CAD(IS,:)))*DT4D2/DDIR
           IF (CFLCAD .LT. THR) CYCLE
           ITER = MAX(1,NINT(CFLCAD))
           DT4DI = DT4D2 / REAL(ITER)
           DO IT = 1, ITER ! Iteration
             CALL QUICKEST_DIR(MDC,LCIRD,ACQ,CADS,DT4DI,DDIR)
           END DO          ! end Interation
           DO ID = 1, MDC
             TMP = ( ACQ(ID) - AC2(IP,IS,ID) ) /  DT4D2  
             IF ( TMP .GT. 0.) THEN
               IMATRA(IS,ID) = IMATRA(IS,ID) + TMP
             ELSE
               IMATDA(IS,ID) = IMATRA(IS,ID) - TMP/MAX(SMALL,AC2(IP,IS,ID)) 
             END IF
           END DO
         END DO
         RETURN
...
!$OMP&         SHARED(AC2,SI,DAC_SPE,DT4D,DDIR,LCIRD,LITERSPLIT,MNP, & 
!$OMP&                MSC,MDC,WK,DEP,DMIN,CURTXY,ICOMP,IOBP)  &
         ...
         SELECT CASE (DIMMODE)
             ...
             CASE (1)
              ...
              CASE (2)
               ...
               !WRITE(*,*) MAXVAL(DDEP(:,1)), MAXVAL(DDEP(:,2))
               !WRITE(*,*) MINVAL(DDEP(:,1)), MINVAL(DDEP(:,2))
