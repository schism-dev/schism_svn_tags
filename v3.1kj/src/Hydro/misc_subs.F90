                ...
                if(ie/=0) then
                  ...
                  do jj=1,3 !side; in the aug. domain
                    ...
                    if(idry_s(isd)==0) then
                      ...
                      su2(k,i)=su2(k,i)+swild2(1) !su2(k,isd)
                      sv2(k,i)=sv2(k,i)+swild2(2) !sv2(k,isd)
!                      tsd(k,i)=tsd(k,i)+tsd(k,isd)
!                      ssd(k,i)=ssd(k,i)+ssd(k,isd)
                      ttmp=ttmp+tsd(k,isd)
                      stmp=stmp+ssd(k,isd)
...
!LLP end
     &                 )
      use elfe_glbl, only: rkind,tempmin,tempmax,saltmin,saltmax,errmsg,ifort12,ntracers
      use elfe_msgp, only : parallel_abort
      implicit real(rkind)(a-h,o-z),integer(i-n)
...
!       Currently bounds not enforced
     ...
     &(/xcj(i),ycj(i),zcj(i)/),sframe(:,:,i),xn2,yn2,tmp)
          ...
          !delta1=(xnd(node2)-xnd(node1))*y43-x43*(ynd(node2)-ynd(node1))
...
!     Init. for outputs
      ...
      if(wfr<1.e-4.or.ubm<1.e-4) then 
...
!      Ubm = Wheight*wr/Sinh(Wnum*Depth) !orbital vel.
      ...
      if(rkn==0) then
        ...
        tmp=5.61*(c_mu*ubm/rkn/wfr)**(-0.109)-7.3
      endif
      if(tmp>500) call parallel_abort('WBL: exponent too large')
      fw=c_mu*exp(tmp)
...
!            write(*,*)'wave bottom layer did not converge:',rmu,rmu2,tau_wm,fw,ubm,phi_cw,c_mu,wfr
          ...
          cm_ubm=c_mu*ubm/rkn/wfr
          tmp=5.61*cm_ubm**(-0.109)-7.3
          if(tmp>500) call parallel_abort('WBL: exponent too large (2)')
          fw=c_mu*exp(tmp)
