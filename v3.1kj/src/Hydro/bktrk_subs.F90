...
!TODO: for ics=2, akrmat_nd is based on eframe and need to project vel. to this frame 1st
...
!       uuint0,vvint0,wwint0: vel. used in 'time' (info only)
...
!      uuint,vvint,wwint: interpolated vel. at end pt 
!********************************************************************************
!
      use elfe_glbl
      ...
      real(rkind) :: wild(10,2),wild2(10,2) !,xy_l(3,2)
      real(rkind) :: vxl(3,2),vyl(3,2),vzl(3,2),vxn(3),vyn(3),vzn(3)
...
!         end pt (xt,yt) is on a side it's nudged into element to conclusion
!         the process - this wob't create CPU dependency
...
!        if(ics==1) then
!          xn1=xnd(n1); yn1=ynd(n1)
!          xn2=xnd(n2); yn2=ynd(n2)
!        else !lat/lon
!          call project_pt('g2l',xnd(n1),ynd(n1),znd(n1),gcor0,frame0,xn1,yn1,zn1)
!          call project_pt('g2l',xnd(n2),ynd(n2),znd(n2),gcor0,frame0,xn2,yn2,zn2)
!        endif !ics
!        wild(i,1)=signa(xn1,xn2,x0,yn1,yn2,y0)
!        wild(i,2)=signa(xn1,xn2,xt,yn1,yn2,yt)
!        xy_l(i,1)=xn1; xy_l(i,2)=yn1 !for debuggging below
!      enddo !i
!      ar_min1=minval(wild(1:3,1))/area(nel)
!      ar_min2=minval(wild(1:3,2))/area(nel)
      !Assume init. pt inside nnel
!      if(ar_min1<0) then
!        write(errmsg,*)'QUICKSEARCH: (x0,y0) not in nnel initially',l_ns,ipsgb,ielg(nnel), &
!     &ar_min1,wild(1:3,1),x0,y0,xt,yt,xy_l(1:3,1),xy_l(1:3,2),gcor0,frame0,time,trm
!        call parallel_abort(errmsg)
!      endif

      !if(ar_min2>-small1) then
!      if(ar_min2==0) then
!        write(errmsg,*)'QUICKSEARCH impossible(1):',l_ns,ipsgb,ielg(nel),ielg(nnel00), &
!     &x0,y0,xt,yt,time
!        call parallel_abort(errmsg)
!      endif !ar_min2==0
      ...
      ar_min2=minval(arco)
!      if(ar_min2==0) then 
!        write(errmsg,*)'QUICKSEARCH impossible(1):',idx,itr,l_ns,ipsgb,ielg(nel),jlev00, &
!     &x0,y0,xt,yt,time,uuint0,vvint0,wwint0
!        call parallel_abort(errmsg)
!      endif
      if(ar_min2>-small1) then
