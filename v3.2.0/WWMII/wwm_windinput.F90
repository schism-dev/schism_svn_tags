...
!
         ...
         REAL              :: WINDX, WINDY, VEC2RAD

         WINDX  = WINDXY(IP,1)
         WINDY  = WINDXY(IP,2)
         WIND10 = MAX(1.,SQRT(WINDX**2+WINDY**2))
...
!
                DO I = 1, MAXITER
                  ...
                  z0_t = MAX( THR, 0.1*(VISK/MAX(THR,UFRIC1)) + ( CZ0T + fU10 ) * UFRIC1**2/G9 )      ! Eq. 7
                  TAUHF(IP) = (KAPPA**2*WIND10**2) / LOG(10.0/z0_t)**2          ! Eq. 8
                  z00  = 10. * EXP( -( KAPPA*WIND10 / MAX(THR,UFRIC1) ) )         ! Estimate the roughness length according the log. profile
                  ULur = UFRIC1/KAPPA * LOG ((EPS_B/KP_w)/MAX(THR,z00))           ! Estimate the velocitiy in the height of reference
...
!      Janssen input from the WAM source code ... 
!
         USE DATAPOOL, ONLY : WINDXY, DELTAUW, DELU, ITAUMAX, JUMAX, TAUT, THR , UFRIC, Z0, TAUTOT, G9
       ...
       IMPLICIT NONE
         ...
         DO IS = 1,MSC
           ...
           DO ID = 1, MDC
             ...
             IF (SSINE(IS,ID) .GT. 0.) LWINDSEA(IS,ID) = .TRUE.
...
!
        ...
        INTEGER :: L, K, J
        REAL    :: ALPHAM, CONST1, OMEGAC, X0, UST, Z00, ALPHAMCOEF
