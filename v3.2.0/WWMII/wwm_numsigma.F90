...
!$OMP&         SHARED(AC2,WK,DEP,CURTXY,DS_BAND,DS_INCR,IOBP,DT4F,IITERSPLIT, & 
!$OMP&         DDIR,PTAIL,MNP,MSC,MDC,DMIN,DAC_SIG,DAC_ADV,DAC_THE,DAC_SOU) PRIVATE(IP,IS,DT4FI, &
...
!                 DAC_SIG(IP,:,ID) = (MAX(0.,REAL(ACQ(1:MSC))) - DBLE(AC2(IP,:,ID)))
!                 AC2(IP,:,ID) = MAX(0.,REAL(ACQ(1:MSC)))
!               ELSE
!                 DAC_SIG(IP,:,ID) = (MAX(0.,REAL(ACQ(1:MSC))) - DBLE(AC2(IP,:,ID)))
!                 AC2(IP,:,ID) = MAX(0.,REAL(ACQ(1:MSC))) - DAC_ADV(IP,:,ID) - DAC_THE(IP,:,ID) - DAC_SOU(IP,:,ID)
!               END IF
!             ELSE
               AC2(IP,:,ID) = MAX(0.,REAL(ACQ(1:MSC)))
!             END IF
