...
!**********************************************************************
      REAL FUNCTION BOTFC(KH)
        ...
        BOTFC = REAL(AUX/MAX(DBLE(VERYSMALL),AUX1) - KH*TANH(KH)/MAX(DBLE(VERYSMALL),(2.*(COSHKH)**2)))
...
!**********************************************************************
      REAL FUNCTION BOTFS(KH)
        USE DATAPOOL, ONLY : THR, VERYSMALL
#ifdef SELFE
        use elfe_glbl, only: errmsg
        use elfe_msgp
#endif
        ...
        IF (ABS(DCOSH(DKH)) > DBLE(VERYSMALL)) THEN
          SECH = 1. / DCOSH(DKH)
        ELSE
          SECH = 0. 
        END IF
        ...
        AUX     = SECH**2/MAX(DBLE(VERYSMALL),(6.d0*(2.d0*DKH+SINH2KH)**3))
        AUX1    = 8.d0*(DKH**4)+16.d0*(DKH**3)*SINH2KH- &
              &   9.d0*(SINH2KH**2*COSH2KH+12.d0*DKH*(1.d0+2.d0*(SINHKH)**4)* &
              & (DKH+SINH2KH))
        BOTFS = REAL(AUX * AUX1)
        ...
        IF (BOTFS .NE.  BOTFS) THEN
          ...
          WRITE(*,*) 9.d0*(SINH2KH**2*COSH2KH+12.d0*KH*(1.d0+2.d0*(SINHKH)**4)*(KH+SINH2KH))
          STOP 'BOTFS'
#endif
        ENDIF
...
!**********************************************************************
      REAL FUNCTION BOTFC2(KH)
        USE DATAPOOL, ONLY : VERYSMALL, VERYLARGE
        IMPLICIT NONE
        REAL, INTENT(IN) :: KH
        REAL :: AUX, AUX1 
        ...
        AUX = -4.0*KH*COSHKH + SINH3KH + SINHKH + 8.0*(KH**2)*SINHKH
        AUX1 = 8.0*COSHKH**3.0*(2.*KH + SINH2KH)
        BOTFC2 = AUX / MAX(VERYSMALL,AUX1) - KH*TANH(KH) / (2.0*(COSHKH)**2)

        IF (BOTFC2 .NE. BOTFC2) THEN
           WRITE(*,*) 'BOTFC2'
           WRITE(*,*) SINHKH, COSHKH, SINH2KH, SINH3KH, KH
           WRITE(*,*) AUX, AUX1, KH*TANH(KH), (2.0*(COSHKH)**2)
           STOP  'BOTFC2'
        ENDIF
...
!**********************************************************************
      REAL FUNCTION BOTFS2(KH)
        ...
        SECH = 1.0 / MAX(VERYSMALL,COSHKH)
        AUX = SECH**2 / MAX(VERYSMALL, (6.0*(2.0*KH + SINH2KH)**3.0))
        IF (AUX .GT. VERYSMALL) THEN
          AUX1 = 8.0*(KH**4.0) + 16.0*(KH**3.0)*SINH2KH &
          &   - 9.0*(SINH2KH)**2*COSH2KH &
      ...
      SUBROUTINE DIFFRA_EXTENDED()
         ...
         DO IP = 1, MNP
            ...
            IF ( AUX*ENG(IP) .GT. VERYSMALL) THEN
               ...
               END IF
               CAUX2 = CAUX * CAUX
               DELTA = CAUX2*(1.0+CAUX)**2-NAUX*(CAUX2-NAUX)*(1.0+(DFWAV+DFBOT(IP)+DFCUR(IP))/AUX+DFCUT)
               IF (DELTA <= 0.0) THEN
                 DIFRM(IP) = 1.0
               ELSE
