...
! 
         ...
         IF (DEP > DMIN) THEN
            SGDLS = SIGIN*SIGIN*DEP/G9
            AUX1 = 1.d0+0.6522d0*SGDLS+0.4622d0*(SGDLS**2)+0.0864d0*(SGDLS**4)+0.0675d0*(SGDLS**5)
...
!*                                                                    *
!**********************************************************************
      SUBROUTINE ERROR_MSG(MESG,SUBNAME)
         IMPLICIT NONE
         CHARACTER(LEN=*) :: MESG
         CHARACTER(LEN=*) :: SUBNAME

         WRITE(*,*) MESG 
         WRITE(*,*) 'THE ERROR IS IN FOLLOWING SUBROUTINE', SUBNAME 
         STOP 'ERROR_MSG'
...
!*                                                                    *
!**********************************************************************
      SUBROUTINE INTELEMENT_AC_LOC(IE,XPC,YPC,ACLOC,CURTXYLOC,DEPLOC,WATLEVLOC,WKLOC)
      USE DATAPOOL
      IMPLICIT NONE
      INTEGER, INTENT(IN)  :: IE
      REAL,    INTENT(IN)  :: XPC, YPC
      REAL,    INTENT(OUT) :: ACLOC(MSC,MDC)
      REAL,    INTENT(OUT) :: CURTXYLOC(2), DEPLOC, WATLEVLOC, WKLOC(MSC)
      ...
      CALL INTELEMENT(XOUTELE,YOUTELE,WATLEV(NI),XPC,YPC,WI,WATLEVLOC,LSAME)
      ...
      SUBROUTINE CSEVAL ( NFU, FILEN, LFT, LSE, NPT, DIMS, SEVAL, DTSE, DTML, DEVAL )
         ...
         IF (LSE) THEN
            ...
            DO IC = 1, DIMS
               ...
               IF ( IFSTAT /= 0 ) CALL ERROR_MSG('unexpected error reading the serial file','CSEVAL')
            END DO
         ELSE
