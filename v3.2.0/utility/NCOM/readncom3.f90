...
!          write(88,*)i,xcj(i),ycj(i),ssd(i,1),ssd(i,nvrt)
        ...
        ihot_len=nbyte*(5+6*mnv+(2*nvrt+1)*ne+(8*nvrt+1)*ns+3*np+12*np*nvrt+1)+12
        open(36,file='hotstart.in',access='direct',recl=ihot_len)
        write(36,rec=1)0,0.d0,1,mnv,(0.d0,10.d0,0.d0,k=1,mnv),(0,(0.d0,j=1,nvrt),i=1,ne), &
     &(0,(0.d0,0.d0,dble(tsd(i,j)),dble(ssd(i,j)),j=1,nvrt),i=1,ns), &
     &(0.d0,0,(dble(tempout(i,j)),dble(saltout(i,j)),dble(tempout(i,j)),dble(saltout(i,j)),0.d0,0.d0,j=1,nvrt),i=1,np), &
     &1,'1           '
