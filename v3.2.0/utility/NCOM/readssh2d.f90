...
!
! ifort -g -O3 -Bstatic -o readssh2d_canopus readssh2d.f90 -Vaxlib -I/usr/local/include -L/usr/local/lib -lnetcdf
!
...
!     (2) hgrid.ll;
!     (3) date.in: year, month, day, # of days; 2nd line: ireduce; 
...
!                  at invalid pts
!     (2) Z0.th: time history of Z0 at the specified open bnds. "Invalid" if at least one of 
!                parent nodes is dry. Use gen_elev3D.f90 to generate elev3D.th from Z0.th.
...
!      dimension tsd(mns,mnv),ssd(mns,mnv)
      ...
      dimension nond(mnope),iond(mnope,mnond),isbnd(mnp),iob(mnope),ival(mnp)
