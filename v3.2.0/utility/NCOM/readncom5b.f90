...
!     (6) blend.gr3: weighting factors for salt (1: full NCOM);
!   Output: hotstart.in or salt_nu.in and temp_nu.in
...
!     integer :: i,j,k,i1,i2,i3,j1,j2,j3
      ...
      dimension ztot(mnv),sigma(mnv),cs(mnv),z(mnp,mnv),iest(mnp),ixy(mnp,2),arco(3)
      ...
      dimension xcj(mns),ycj(mns),blend(mnp)
...
!     Read in hgrid and vgrid
      ...
      open(24,file='blend.gr3',status='old')
      ...
      do i=1,np
        ...
        read(24,*)j,xtmp,ytmp,blend(i)
        if(blend(i)<0.or.blend(i)>1) then
          write(11,*)'Wrong weighting factor for S: ',i,blend(i)
          stop
        endif
        !blend(i)=max(0.,min(1.,(dp(i)-800)/(1500-800)))
        !write(25,*)i,xcor(i),ycor(i),blend(i)
...
!     In case kz=1, there is only 1 ztot(1)=-h_s
      ...
      if(h_c<10) then !large h_c to avoid 2nd type abnormaty
...
!     ireduce=0: original set; =1: reduced bg set for quicker interpolation (edit ixlen[1,2],iylen[1,2])
      read(10,*) ireduce 
!      if(ndays>20) then
!        write(11,*)'No more than 20 days pls!'
!        stop
!      endif
      close(10)
...
!          write(12,*)(i-1)*iylen1+j,lon(i,j),lat(i,j),kbp(i,j)
        enddo !j
      enddo !i  
...
!         if(kbp(i,j)/=-99) then
!           write(15, *) (i-1)*iylen1+j,lon(i,j),lat(i,j),salt(i,j,1) !-h(i,j) 
!         endif
!        enddo
!      enddo
...
!          write(13, *)(i-1)*iylen+j,lon(i,j),lat(i,j),salt(i,j,1) !-h(i,j) 
...
!                 Enforce lower bound for salt (this is the only occurence in the code)
                  salt_min=max(31.,min(33.,31-2*z(i,k)/4))
                  tmp=max(salt_min,saltout(i,k))
                  saltout(i,k)=saltout(i,k)*blend(i)+tmp*(1-blend(i))
                enddo !k=1,nvrt
              endif
              cycle loop4
            endif !rat<small1
...
!-----------------------------------------------------------------------------------------------------------
      ...
      do i=1,np
        write(20,*)i,xcor(i),ycor(i),saltout(i,18)
        write(21,*)i,xcor(i),ycor(i),saltout(i,nvrt)
      enddo !i
