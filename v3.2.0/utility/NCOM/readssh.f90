...
!     (2) hgrid.ll;
!     (3) date.in: year, month, day, # of days (=0); 2nd line: ireduce
!   Output: 
!     (1) ssh.ll (in lat/lon; ssh=-99 where parent triangle is dry
!     (2) fort.99: original SSH on the lat/lon from NRL; build pts. This can be used to "extrapolate" ssh
!                  at invalid pts
...
!      open(19,file='vgrid.in',status='old')
      ...
      read(16,*)
      read(16,*)
      write(20,*)'SSH on hgrid.ll (-99 for invalid pts)'
      write(20,*)ne,np
      do i=1,np
...
!              endif
              ...
              etaout(i)=-99 
              if(in==1.and.h(ix,iy)/=0.and.h(ix+1,iy)/=0.and.h(ix+1,iy+1)/=0) then
                etaout(i)=ssh(ix,iy)*arco(1)+ssh(ix+1,iy)*arco(2)+ssh(ix+1,iy+1)*arco(3)
              else if(in==2.and.h(ix,iy)/=0.and.h(ix+1,iy+1)/=0.and.h(ix,iy+1)/=0) then
                etaout(i)=ssh(ix,iy)*arco(1)+ssh(ix+1,iy+1)*arco(2)+ssh(ix,iy+1)*arco(3)
              endif
              cycle loop4
            endif !rat<small1
