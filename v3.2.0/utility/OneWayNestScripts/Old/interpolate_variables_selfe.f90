...
! 	       fg.bp: grid points that need interpolation from bg.gr3 (with     *
!		      correct depths);
!	       vgrid.fg: vgrid for fg.bp (must be pure S SELFE);		*
!              interpolate_variables_elcirc.in: 1st line: ifiletype (1: elev; 2: S,T); ndays;
!					        2nd line: t_bk,s_bk (back-up T,S for dry spots);	
!       Output: fort.17 (temp.); fort.18 (salt); or fort.16 (elev.)		*
!										*
!********************************************************************************
!
      program interpolate
...
!...
      ...
      read(21,*)t_bk,s_bk !back-up values for T,S for dry spots
      if(ifiletype==1) then
        file63='elev.61'
      else
        file63='temp.63'
...
!          print*, 'Day ',iday
          do i=1,npfg
            ...
            if(kbe==0) then
              write(*,*)'All dry'
!              stop
               tfg(i,1:nvrt_fg)=t_bk
               sfg(i,1:nvrt_fg)=s_bk
               cycle
            endif
            do k=kbe,nvrt
              ze(k)=-1.e6
