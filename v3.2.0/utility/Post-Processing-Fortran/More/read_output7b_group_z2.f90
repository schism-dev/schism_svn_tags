...
!****************************************************************************************
!											*
!	read_output7b with (x,y) read in from station.bp (build points) 
!       or station.sta (sta format; x,y, with z), at z values
!        read in from the depths in station.bp/.sta (not used for 2D variables).
!       Outputs: fort.1[89]; fort.20 - local dapth for each pt.
!       For ics=2 (lat/lon), use nearest node for output

!       Compile: ifort -Bstatic -assume byterecl -O3 -o read_output7b_group_z2 read_output7b_group_z2.f90
!  PGI compiler:
!  pgf90 -O2 -mcmodel=medium  -Bstatic -o read_output7b_group_z2 read_output7b_group_z2.f90
!											*
!****************************************************************************************
!
      program read_out
...
!      dimension ztmp(mnv),x00(mnp),y00(mnp),iep(mnp),swild(3),out3(mnp,2),z00(mnp)
      ...
      print*, 'Input offset (days) for start time:'
      read(*,*) start_corie
      ...
      do i=1,nxy
        if(ibp==1) then !.bp format
          ...
          read(10,*)
          read(10,*)x00(i),y00(i),z00(i) !z00 ==0 is MSL; z00<0 is below MSL
        endif !ibp
      enddo !i
      close(10)
...
!        enddo !i
        ...
        if(i23d.eq.2) then
          ...
          enddo !i
          irec=irec+np*ivs
          write(18,'(e16.8,6000(1x,f12.3))')time/86400+start_corie,(out2(i,1,1),i=1,nxy)
          if(ivs==2) write(19,'(e16.8,6000(1x,f12.3))')time/86400+start_corie,(out2(i,1,2),i=1,nxy)
        else !i23d=3 
          do i=1,nxy
            do j=1,3 !nodes
              nd=node3(i,j)
...
!             Interplate in vertical
              ...
              enddo !k
              if(k0==0) then
                write(*,*)'Warning: failed to find a vertical level:',iday,it,i,ztmp(kbpl),ztmp(nvrt)
                stop
!                Default for out3 is 0.
          ...
          enddo !i=1,nxy
          write(18,'(e16.8,6000(1x,f14.5))')time/86400+start_corie,(out3(i,1),i=1,nxy)
          if(ivs==2) write(19,'(e16.8,6000(1x,f14.5))')time/86400+start_corie,(out3(i,2),i=1,nxy)
