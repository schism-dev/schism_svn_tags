...
!       add extra times b4 and after to examine phase errors.
!       Inputs: (1) binary files;
!               (2) station.xyt: make sure all times are after 1st record (to ensure interpolation in time); 
!                                 pad extra days before and after if necessary.
!               (3) read_output7b_xyt.in: 
!                   file63\n 
!                   invalid value (for out of domain, dry etc)\n 
!                   start corie day \n
!                   window (hours) b4 abnd after the cast, stride (hrs) - used to 
...
!                The # of 'virtual' casts for each actual cast is 2*window/stride+2
!											*
!       ifort -Bstatic -assume byterecl -O2 -o read_output7b_xyt read_output7b_xyt.f90
!****************************************************************************************
!
      program read_out
      parameter(nbyte=4)
...
!      dimension ztmp(mnv),x00(mnp2),y00(mnp2),iep(mnp2),out4(mnv,2),t00(mnp2)
...
!       Interpolate in time
        ...
        if(i23d==2) then
          ...
          do k=nvrt,kbpl,-1
            ...
            write(18,'(i6,3(1x,f12.3))')i,out4(k,1),ztmp(k)-ztmp(nvrt),t00(i)/86400+start_corie 
            if(ivs==2) write(19,'(i6,3(1x,f12.3))')i,out4(k,2),ztmp(k)-ztmp(nvrt),t00(i)/86400+start_corie
          enddo !k
        endif
      enddo !i=1,nxy
