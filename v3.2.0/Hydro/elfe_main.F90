...
!                              All Rights Reserved                                      !
!               THIS IS A PRIVIATE COPY INTENDED FOR RESTRCITED USERS; UNAUTHORIZED
...
! Message passing arrays
      integer,allocatable :: srqst(:),sstat(:,:),rrqst(:),rstat(:,:)
      integer,allocatable :: nhtsend1(:),nhtrecv1(:),ihtsend1(:,:),ihtrecv1(:,:)
      integer,allocatable :: ihtsend1_ndgb(:,:),ihtrecv1_ndgb(:,:)
      integer,allocatable :: htsend_type(:),htrecv_type(:)
      integer,allocatable :: send_bsize(:),recv_bsize(:) !block size
      allocatable :: block_refnd2_eta(:)
...
!      allocatable :: iet1lg(:),ifl1lg(:),ite1lg(:),isa1lg(:)
      allocatable :: iblock_pairs(:),isblock_nd(:,:),isblock_el(:),iblock_nd2_ref_gb(:,:)
      allocatable :: q_block(:),vnth_block(:,:),iblock_nd_gb(:,:,:),dir_block(:,:)
      allocatable :: q_block_lcl(:),iq_block_lcl(:),iq_block(:)
...
!     Wild-card arrays
      ...
      allocatable :: buf1(:,:),buf2(:,:),buf3(:),ibuf1(:,:),ibuf2(:,:)
...
! Some param. from WWM for elfe_msgp
#ifdef USE_WWM
      call get_param('msc2',1,msc2,realvalue,stringvalue)
      call get_param('mdc2',1,mdc2,realvalue,stringvalue)
#endif
...
!     Allocate the remaining grid geometry arrays held in elfe_glbl
      allocate(kbe(nea),idry_e(nea),idry_e0(nea),lqk(nea),ie_kr(nea), &
     &krvel(nea),itvd_e(nea),ze(nvrt,nea),dl(nea,3,2),dp00(npa),kfp(npa),kbp(npa), &
     &kbp00(npa),kbp_e(np),idry(npa),iback(npa),hmod(npa),znl(nvrt,npa), &
     &kbs(nsa),idry_s(nsa),isidenei2(ns,4),zs(nvrt,nsa),side_ac(nsa,2,2), &
     &side_x(nsa,2),delj(ns),ibnd_ext_int(npa),pframe(3,3,npa),stat=istat)
...
!          Note: swild, swild2, swild10 will be re-dimensioned (larger dimension) later
           ...
           PSQ(nea),PSK(nea),surf_t1(npa),surf_t2(npa),surf_t(npa),stat=istat)
...
!     Wave model arrays
#ifdef  USE_WWM
      allocate(wwave_force(nvrt,nsa,2),out_wwm(npa,34),out_wwm_windpar(npa,9),stat=istat)
...
!...  Read in hydraulics.in
      call get_param('ihydraulics',1,ihydraulics,realvalue,stringvalue)
...
!'
        open(31,file='hydraulics.in',status='old')
...
!'
        if(nhtblocks>0) then
          allocate(iblock_pairs(nhtblocks),isblock_nd(2,npa),dir_block(3,nhtblocks), &
     &isblock_sd(2,nsa),isblock_el(nea),q_block(nhtblocks),vnth_block(2,nhtblocks), &
     &iblock_nd2_ref_gb(2,nhtblocks),q_block_lcl(nhtblocks),iq_block_lcl(nhtblocks), &
     &iq_block(nhtblocks),stat=istat)
          if(istat/=0) call parallel_abort('MAIN: Alloc failed (9)')
          do i=1,nhtblocks
            read(31,*)k,iblock_pairs(i),iblock_nd2_ref_gb(1:2,i) !block #, # of node-pairs, 2 ref. nodes (global indices) for 2 faces
            if(iblock_pairs(i)<2) call parallel_abort('MAIN: Too few pairs')
!'
            do j=1,iblock_pairs(i); read(31,*); enddo;
          enddo !i
          mnpairs=maxval(iblock_pairs)
          allocate(iblock_nd_gb(2,mnpairs,nhtblocks),stat=istat)
          if(istat/=0) call parallel_abort('MAIN: Alloc failed (13)')
          rewind(31)
          read(31,*); read(31,*)
          isblock_nd=0 !init.
          do i=1,nhtblocks
            read(31,*)
            do j=1,iblock_pairs(i)
              read(31,*)iblock_nd_gb(1:2,j,i) !global index
              do k=1,2
                nd_gb=iblock_nd_gb(k,j,i)
          ...
          do i=1,ne
            jblock=minval(isblock_nd(1,nm(i,1:3)))
            if(jblock>0) then
              ...
              do j=1,3 !side
                ...
                if(isblock_nd(1,n1)/=isblock_nd(1,n2)) then
                  write(errmsg,*)'Check block nodes (0):',iplg(isidenode(isd,1:2))
                  call parallel_abort(errmsg)
                endif
                if(isblock_nd(2,n1)==2.and.isblock_nd(2,n2)==2) then
                  swild99(1:3,jblock)=swild99(1:3,jblock)+sframe(1:3,1,isd)*ssign(i,j)
...
!USE_TIMOR
!Aron:  init. condition for TIMOR below if you don't want to use standard format
!In all tracer modules, these are the arrays used:
!       trel(ntracers,nvrt,nea): tracer concentration at prism centers;
!       trel0(ntracers,nvrt,nea): i.c. for concentrations
!       tr_el(mntr,nvrt,nea): used as temporary array to store some temp. values (mntr=max(ntracers,2)
!       bdy_frc(mntr,nvrt,nea): source/sink at prism centers;
!       flx_sf(mntr,nea),flx_bt(mntr,nea): surface/bottom boundary conditions
!       The transport eq. is:
!       DC/Dt=d/dz(\kappa*dC/dz)+bdy_frc
!       with b.c.:
!       \kappa*dC/dz = flx_sf at surface; \kappa*dC/dz = flx_bt at bottom
...
!                      BASIC_PARAMETER() in wwm_initio.F90 for details; below is a snapshot from there:
      ...
      sum1=sum(out_wwm_windpar(1:npa,1:9))
      sum2=sum(wwave_force)
      sum3=sum(out_wwm(:,1:34))
      ...
      if(icou_elfe_wwm>0.and.iwind_form==-2) then 
        do i=1,npa
          if(idry(i)==1) then
            ...
            tau(i,1)=tmp*cos(theta)
            tau(i,2)=tmp*sin(theta)
          endif
        enddo !i
      endif !icou_elfe_wwm
#endif
...
!...  in case the blocks are taken out
      if(ihydraulics/=0.and.nhtblocks>0) then
        !Read in fluxes
        !read(49,*)ttt,q_block(1:nhtblocks) !dir is from face 1 to 2
        !if(it==iths+1.and.abs(ttt-time)>1.e-4) then
        !  write(errmsg,*)'Starting time wrong for blocks',it,ttt
        !  call parallel_abort(errmsg)
        !endif
...
!'
        ...
        do i=1,nhtblocks
          ...
          if(ipgl(ndgb1)%rank==myrank) then;  if(ipgl(ndgb1)%id<=np) then
            ...
            !Compute flux
            call calc_struc_flow(i,eta2(ipgl(ndgb1)%id),block_refnd2_eta(i),q_block_lcl)
            iq_block_lcl(i)=iq_block_lcl(i)+1

            !Test; q_block is vnth_block
            !diff=eta2(ipgl(ndgb1)%id)-block_refnd2_eta(i)
            !q_block_lcl(i)=0.6*sqrt(2*grav*abs(diff))*sign(1.d0,diff) !vel. from "1" to "2"
          endif; endif !ipgl
        enddo !i=1,nhtblocks
          ...
          if(rough_p(i)<=0) then !time-independent Cd
           ...
           if(bthick<=rough_p(i)) then
              ...
              if(iwbl==1) then
                ...
                wfr=2*pi/max(0.1,out_wwm(i,11)) !angular freq.; out_wwm is not real*8
...
!            etam=(eta2(n1)+eta2(n2))/2
            ...
            if(hhat(isd)<=0.or.htot<=0) then
              write(errmsg,*)'Impossible dry 51:',htot,hhat(isd),ielg(i),iplg(isidenode(isd,1:2))
              call parallel_abort(errmsg)
            endif
        ...
        if(ihydraulics/=0.and.nhtblocks>0) then; if(isblock_nd(1,i)>0) then
          do j=1,nne(i)
            ...
            do m=1,lim
              ...
              if(i/=isidenode(isd,1).and.i/=isidenode(isd,2)) call parallel_abort('MAIN: impossible 51')
...
!	Impose b.c.
        do k=kbs_e(j),nvrt
          ...
          if(ihydraulics/=0.and.nhtblocks>0) then; if(isblock_sd(1,j)>0) then
            ...
            !Check
            !write(12,*)'Vel. b.c:',iplg(isidenode(j,1:2)),k,jblock,jface,real(su2(k,j)),real(sv2(k,j))
          endif; endif !ihydraulics
        enddo !k=kbs_e(j),nvrt
      enddo !j=1,nsa
...
!            tr_el(1:ntracers,:,:)=trel(1:ntracers,:,:) !already init.
            ...
            flx_sf = 0.d0
          case(6) !TIMOR
!Define bdy_frc,flx_bt,flx_sf here
...
!Outputs for TIMOR are *_trcr_[1,2,...,ntracers].63 (3D scalars only)
!We'll need to add 2D scalar, 2D/3D vectors outputs for you if needed
#endif /*USE_TIMOR*/
