...
!... ic3(i,j) shows the element # of neighbouring element.
!... ic3(i,j) = 0 shows no neighbouring element, it is boundary.
...
!...  compute hvis_e & hvis_e based on Smagorinsky Algorithm
      ...
      do i=1,ne
          ...
          do j=1,3
            ...
            if(m1>3) m1=m1-3
            if(m2>3) m2=m2-3            
            dux=dux+uu2(nm(i,j),k)*(ycp(m1)-ycp(m2))
            duy=duy+uu2(nm(i,j),k)*(xcp(m2)-xcp(m1))
            dvx=dvx+vv2(nm(i,j),k)*(ycp(m1)-ycp(m2))
            dvy=dvy+vv2(nm(i,j),k)*(xcp(m2)-xcp(m1))
          enddo                  
