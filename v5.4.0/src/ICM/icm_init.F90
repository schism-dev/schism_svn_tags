...
!--------------------------------------------------------------------------------
  ...
  allocate(wqc(ntrs(7),nvrt,nea),TIC(nvrt,2),ALK(nvrt,2),CACO3(nvrt,2),CA(nvrt,2),PH(nvrt), & !PH model variables 
    & CAsat(nvrt),CO2(nvrt),PH_el(nvrt,nea),PH_nd(nvrt,npa),iphgb(ne_global),ph_nudge(nea),ph_nudge_nd(npa), &
    & TIC_el(nvrt,nea),ALK_el(nvrt,nea), &
    & dep(nvrt),Sal(nvrt),Temp(nvrt),TSED(nvrt),ZB1(nvrt,2),ZB2(nvrt,2),PB1(nvrt,2), &
    & PB2(nvrt,2),PB3(nvrt,2),RPOC(nvrt,2),LPOC(nvrt,2),DOCA(nvrt,2),RPON(nvrt,2),LPON(nvrt,2), &
    & DON(nvrt,2),NH4(nvrt,2),NO3(nvrt,2),RPOP(nvrt,2),LPOP(nvrt,2),DOP(nvrt,2),PO4t(nvrt,2), &
    & SU(nvrt,2),SAt(nvrt,2),COD(nvrt,2),DOC(nvrt,2),GP(nvrt,3),PrefN(nvrt,3),PC2TSS(nea), &
    ...
    & WWPDO(nea), WWPSalt(nea),stat=istat) 
  ...
  allocate(SFA(nea),SED_BL(nea),ZD(nea),SED_B(nea,3),SED_LPOP(nea),SED_RPOP(nea),SED_LPON(nea),SED_RPON(nea), &
      ...
      & WS1BNET(nea),WS2BNET(nea),WS3BNET(nea),WSUBNET(nea), HSED(nea),VSED(nea),VPMIX(nea),VDMIX(nea), &
      & FRPOP(nea,3),FRPON(nea,3),FRPOC(nea,3), flxpop(nea,3),flxpon(nea,3),flxpoc(nea,3), flxpos(nea), &
      & CTEMP(nea),CPIP(nea),CNO3(nea),CNH4(nea),CCH4(nea),CSO4(nea),CPOS(nea), CPOP(nea,3),CPON(nea,3),CPOC(nea,3), &
      ...
      & sbLight(nea), patch(nea),sh(nea),SEDCSAV(nea),SEDNSAV(nea),SEDPSAV(nea),SEDNH4SAV(nea),SEDPO4SAV(nea),SEDDOSAV(nea), &
...
!$OMP parallel workshare default(shared)
  ...
  PB2=0.0;     PB3=0.0;     RPOC=0.0;    LPOC=0.0;    DOCA=0.0;   RPON=0.0;   LPON=0.0
  DON=0.0;     NH4=0.0;     NO3=0.0;     RPOP=0.0;    LPOP=0.0;   DOP=0.0;    PO4t=0.0
  SU=0.0;      SAt=0.0;     COD=0.0;     DOC=0.0;     GP=0.0;     PrefN=0.0;  PC2TSS=0.0
  ...
  WS1BNET=0.0;   WS2BNET=0.0;    WS3BNET=0.0;    WSUBNET=0.0;    HSED=0.0;       VSED=0.0;       VPMIX=0.0;     VDMIX=0.0; 
  FRPOP=0.0;     FRPON=0.0;      FRPOC=0.0;      flxpop=0.0;     flxpon=0.0;     flxpoc=0.0;     flxpos=0.0;
  CTEMP=0.0;     CPIP=0.0;       CNO3=0.0;       CNH4=0.0;       CCH4=0.0;       CSO4=0.0;       CPOS=0.0;      CPOP=0.0;      CPON=0.0; CPOC=0.0;
  ...
  sbLight=0.0;   patch=0.0;      sh=0.0;         SEDCSAV=0.0;    SEDNSAV=0.0;    SEDPSAV=0.0;    SEDNH4SAV=0.0; SEDPO4SAV=0.0; SEDDOSAV=0.0;
