...
!---------------------------------------------------------------------
  ...
  if(iRad/=0.and.time_icm(3)<time) then
...
!---------------------------------------------------------------------
  use schism_glbl, only : rkind,np_global,npa,ne_global,np_global,nea,i34,elnode,ipgl, &
                   & iegl,errmsg
  use schism_msgp, only : myrank, parallel_abort
  use icm_mod, only : iWS,iTurb,iWRea,iTSS,iLight,PC2TSS,WSRP,WSLP,WSPB1,WSPB2,WSPB3,Turb,WRea, &
               & iphgb,iPh,inu_ph,ph_nudge,ph_nudge_nd
  use misc_modules
  implicit none
 
  !local variables
  integer :: i,j,ie,ip,npgb,negb,nd,itmp,itmp1(1),itmp2(1,1)
  real(kind=rkind) :: rtmp,rtmp1(1),rtmp2(1,1),xtmp,ytmp,ztmp
...
!---------------------------------------------------------------------
  ...
  if(iRad/=0) then
    ...
    call parallel_abort('iRad=0, radiation is always needed')
  endif
  ...
  dtw=NDTWQ*dt/86400.0
  dtw2=dtw/2.d0

  !read sediment flux model parameters
  if(iSed==1) call read_icm_sed_param
