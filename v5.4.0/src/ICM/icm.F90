...
!---------------------------------------------------------------------------------
  ...
  if(mod(it,NDTWQ)==0) then
    ...
    do i=1,nea
      ...
      call Phyto(i,hour,nv)

      !add PS and NPS loading, need more work, ZG
      call zeroWPS
      call zeroWNPS
      ...
      !suface renewal rate for DO reareation
      if(iRea==0) then
        ure=0.0; icount=0
        do j=1,i34(i)
          jsj=elside(j,i)
...
subroutine Phyto(id,hour,nv)
!----------------------------------------------------------------------------
!calculate phytoplankton growth rates
!----------------------------------------------------------------------------
  use icm_mod
  use icm_sed_mod, only : sbLight
  use schism_glbl, only : rkind,errmsg,pi,ielg
  use schism_msgp, only : myrank,parallel_abort
  implicit none
  integer, intent(in) :: id,nv
  ...
  !local variables
  integer :: i,j,k
  real(kind=rkind) :: sLight,bLight,mLight,rKe,Chl,rKeh,G(3),xT,rIK,rIs(3),rat
  ...
  if(hour>TU.and.hour<TD) then
    ...
    do k=1,nv
      !adjust maximum growth rate by temperature
      do i=1,3
        xT=Temp(k)-TGP(i)
        if(xT>0.0) then
      ...
      !calculate light attenuation coefficient rKe
      if(iLight==1) then
        rKe=rKeC1*exp(-rKeC2*Sal(k))
      else !iLight==2, iLight==3,iLight=other value
        rKe=Turb(id)+rKeC2*Chl+0.07*TSED(k)
      endif !iLight
     
      rKeh=rKe*dep(k)
      bLight=sLight*exp(-rKeh)
    
      !calculate optimal light intensity 
      ...
      do i=1,3
        ...
        !TIC limitation
        if(iPh==1.and.iphgb(ielg(id))/=0) then
          rtmp=TIC(k,1)**2.d0
...
!----------------------------------------------------------------------------
  use icm_mod
  use schism_glbl, only : rkind,NDTWQ,nvrt,ielg,dt,ne
  use schism_msgp, only : myrank, parallel_abort
  implicit none
  ...
  !calculate depth for the bottom of each layer
  zdep(1)=dep(1)
  do i=2,nv
...
!other zooplanktons, all phytoplankton and carbon species
    ...
    !---------------------------------------------------------------------------------
    if(iPh==1.and.iphgb(ielg(id))/=0) then
      !if(k==1) write(1001,*)ielg(id),iphgb(ielg(id))
       ...
       do m=1,nsta(iid)
         ...
         if((k==1.and.rtmp<=zdep(k).and.rtmp>=0.0).or.(k>1.and.rtmp>zdep(k-1).and.rtmp<=zdep(k))) then
           write(410)time,stanum(m,iid),Temp(k),Sal(k),ZB1(k,1),ZB2(k,1),PB1(k,1),PB2(k,1),PB3(k,1),RPOC(k,1),&
                ...
                & LPOP(k,1),DOP(k,1),PO4t(k,1),SU(k,1),SAt(k,1),COD(k,1),DOC(k,1)
         endif !rtmp
       enddo !m
     endif !ista(i)/=0
   endif !i<=ne
