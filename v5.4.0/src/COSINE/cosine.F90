...
!---------------------------------------------------------------------------
  ...
  !local variables
  integer :: i,j,k,m,iter,id,itrc,iday,daynum
  ...
  real(rkind) :: pnh4s1,pnh4s2,pih1,pih2,rhot,rhop,ADPT,OXR
  real(rkind) :: ph,o2flx,co2flx
  ...
  real(rkind) :: Nit,MIDN,MIDSi

  !Arrays
  real(rkind) :: sLight(1:nvrt+1)
  ...
  do i=1,nea
    ...
    do iter=1,niter    
      ...
      do k=kbe(i)+1,nvrt
        ...
        if(idelay==1 .and. time>=(7.0*86400.d0)) then
          mtime=mod(time/86400.d0,7.d0)
          iday=floor(mtime)+1
        ...
        qcos(5)=NPS2+RPS2-GS2Z2-MTS2+SKS2
        ...
        qcos(9)=(GS2Z2+MTS2)*si2n-MIDSi+SKDSi
        ...
        qcos(10)=-(NPS1+RPS1+NPS2+RPS2)*p2n &
                 &+OXR*(EXZ1+EXZ2)*p2n+OXR*MIDN*p2n
        ...
        !--temperature adjust
        qcos=exp(0.069d0*(temp(k)-TR))*qcos
...
!Output:
!     1)exflux: O2 flux from air to water [m*mol/day.m3]
!-----------------------------------------------------------------
  implicit none
