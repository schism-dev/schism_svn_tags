...
!
call z_fileio(trim(qbase)//'.int','UF',iufind,luq_int,iuerr) ! logging for locus integration
call z_fileio(trim(qbase)//'.trf','UF',iufind,luq_trf,iuerr) ! transformation test
call z_fileio(trim(qbase)//'.t13','UF',iufind,luq_t13,iuerr) ! test output for integration of T13
!
...
!
if (iq_integ==0) call z_fileio(trim(qbase)//'.int','DF',iufind,luq_int,iuerr)
if (iq_test ==0) call z_fileio(trim(qbase)//'.tst','DF',iufind,luq_tst,iuerr)
if (iq_trf  ==0) call z_fileio(trim(qbase)//'.trf','DF',iufind,luq_trf,iuerr)
if (iq_t13  ==0) call z_fileio(trim(qbase)//'.t13','DF',iufind,luq_t13,iuerr)
!
return
end subroutine
!-----------------------------------------------------------------------------!
...
!
!------------------------------------------------------------------------------
if(iq_trf>=1.and.(ik1>=mk1a .and. ik1<=mk1b) .and. (ik3>=mk3a .and. ik3<=mk3b)) then
...
where(t_ia4 > ncirc)
  ...
  if(iq_trf==2) then
    ...
    do iloc=1,nlocusx
      write(luq_trf,'(3i6,4f10.4,e13.5)') iloc,r_ik2(iloc),r_ia2(iloc),&
&     r_w1k2(iloc),r_w2k2(iloc),r_w3k2(iloc),r_w4k2(iloc),r_zz(iloc)
    end do
!
...
!
q_version ='GurboQuad  Version: 5.03 Build: 120 Date: 2004/05/07 [ST]'
!
...
if((ik1>=mk1a .and. ik1<=mk1b) .and. (ik3>=mk3a .and. ik3<=mk3b).and.iq_integ==2)  jq_int=1
...
!!T  end do
!!T end if
if(iq_integ==3 .and. ik1>=30 .and. ik1<=35 .and. ik3>=30 .and. ik3<=35 ) &
&  write(luq_int,'(a,4i3,i5,e13.5)') 'Q_T13V4:',ik1,ia1,ik3,ia3,nlocusx, &
  & t13
!
