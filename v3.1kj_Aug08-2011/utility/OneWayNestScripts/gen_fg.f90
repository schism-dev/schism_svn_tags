!     Generate fg.bp for interpolate_variables_selfe*
!     Input: 
!           hgrid.gr3 (with boundary info); 
!           gen_fg.in (see sample in this dir): 
!                    nob - # of open boundary segments that need *3D.th;
!                    iob(1:nob) - list of # of segment numbers in hgrid.gr3
!     Output: fg.bp

!     ifort -Bstatic -O3 -o gen_fg gen_fg.f90

      implicit real*8(a-h,o-z)
      allocatable :: x(:),y(:),dp(:),nm(:,:),nond(:),iond(:,:),iob(:)
      ...
      read(14,*)
      read(14,*) ne,np
      allocate(x(np),y(np),dp(np),nm(ne,3),stat=istat)
