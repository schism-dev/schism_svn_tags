!     Generate hotstart.in for estuary only runs (MPI version 2.0f and up).

!   Input: 
!     (1) nvrt,nond, ntracers: # of vertical levels; # of nodes at mouth; # of tracers
!     (2) hgrid.gr3
...
!     (4) temp3D.th & salt3D.th (binary): mouth T,S;
!     (5) temp.th (ASCII): river temp.
!   Output: hotstart.in (unformatted)

!   ifort -Bstatic -O3 -assume byterecl -o gen_hot gen_hot.f90
...
!      integer, parameter :: debug=1
      ...
      real*4 dt_th,t3D(mnp,mnv),s3D(mnp,mnv)

      print*, 'Input nvrt,nond:'
      read*, nvrt,nond
!      nvrt=26
!      nond=18
      ntracers=0
...
!     Compute initial S,T
      do i=1,np
        ...
        do k=1,nvrt
          tempout(i,k)=t3D(ianchor0,k)*(1-xrat)+temp_ri*xrat
...
!      write(36)0,0.d0,(0,(0.d0,dble(tsel(j,i,1)),dble(tsel(j,i,2)),j=1,nvrt),i=1,ne), &
...
!      do i=1,np
!        write(20,*)i,x(i),y(i),saltout(i,1)
!        write(21,*)i,x(i),y(i),saltout(i,nvrt)
!      enddo !i
