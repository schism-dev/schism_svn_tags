...
!
         ...
         END IF  
#else
!$OMP PARALLEL DO DEFAULT(NONE) SHARED(AC2,FRHIGH,WINDXY,MSC,CFLCXY,MNP,DEP,DMIN,OUTT,OUT,WIND,CURR,LCFL,IOBP,TIME) PRIVATE(IP,OUTPAR,CURRPARS,WINDPARS,WLM,KLM,ACLOC)
         DO IP = 1, MNP
            IF (DEP(IP) .GT. DMIN) THEN
              ACLOC(:,:) = AC2(IP,:,:)
              CALL INTPAR(IP, MSC, ACLOC, OUTPAR)
...
!$OMP MASTER
         ...
         DO I = 1, 5 
           WRITE(OUT%FHNDL+1)  TIME
           WRITE(OUT%FHNDL+1)  (OUTT(IP,6), OUTT(IP,7), OUTT(IP,I)  , IP = 1, MNP)
           WRITE(OUT%FHNDL+2)  TIME
...
#elif WWMONLY
         REAL :: DEPLOC, WKLOC(MSC), CURTXYLOC(2), ESUM
#endif
         ...
         DO I = 1, IOUTS ! Loop over stations ...
           ...
           IF (LSP1D) THEN
             ...
             ELSE
               ...
               WRITE(OUTSP1D%FHNDL,*) SPSIG, SPDIR, STATION%IFOUND
             END IF
             WRITE(OUTSP1D%FHNDL,*) CTIME, WKLOC, DEPLOC, CURTXYLOC
             DO IS = 1, MSC
               WRITE(OUTSP1D%FHNDL,*) SUM(ACLOC(IS,:))         
             END DO
             CLOSE(OUTSP1D%FHNDL)
           END IF
           ...
           IF (LSP2D) THEN
             ...
             ELSE
               ...
               WRITE(OUTSP2D%FHNDL) SPSIG, SPDIR, STATION%IFOUND
             END IF
             WRITE(OUTSP2D%FHNDL) CTIME, WKLOC, DEPLOC, CURTXYLOC 
         ...
         IF (MYRANK == 0) THEN
           ...
           DO I = 1, IOUTS
             ...
             ELSE 
               ...
               CALL INTPAR_LOC(I ,STATION(I)%ISMAX,WKLOC(I,:),DEPLOC(I),CURTXYLOC(I,:),ACLOC,STATION(I)%OUTPAR_NODE)
             ...
             IF (LSP1D) THEN
               ...
               END IF
               WRITE(OUTSP1D%FHNDL,*) CTIME, WKLOC(I,:), DEPLOC(I), CURTXYLOC(I,:)
               WRITE(OUTSP1D%FHNDL,*) SUM(ACLOC(IS,:)) 
               CLOSE(OUTSP1D%FHNDL) 
             END IF
             ...
             IF (LSP2D) THEN
               ...
               END IF
               WRITE(OUTSP2D%FHNDL) CTIME, WKLOC(I,:), DEPLOC(I), CURTXYLOC(I,:)
               WRITE(OUTSP2D%FHNDL) ACLOC 
               CLOSE(OUTSP2D%FHNDL)
             END IF ! LSP2D
