...
!       Three level splitting to reduce splitting errors when using implicit-explicit schemes
         ...
         CALL CPU_TIME(TIME4)
         CALL COMPUTE_SOURCES_IMP
         CALL CPU_TIME(TIME5)
         CALL COMPUTE_SPATIAL
         CALL CPU_TIME(TIME6)
         IF (LLIMT) CALL ACTION_LIMITER
         IF (.NOT. LDIFR) LCALC = .FALSE.
         CALL CPU_TIME(TIME7)
...
! Do the first guess ... equals the first sub time step of the iteration ...
         ...
         DO IP = 1, MNP
           DO IS = 1, MSC
             DO ID = 1, MDC   
               AC2(IP,IS,ID) =  AC1(IP,IS,ID) + REAL(DAC_ADV(IP,IS,ID))*(IOBPD(ID,IP))*DBLE(IOBDP(IP))
             END DO
           END DO
         END DO
         ...
         DO IP = 1, MNP
           DO IS = 1, MSC
             DO ID = 1, MDC   
               AC2(IP,IS,ID) =  AC1(IP,IS,ID) + REAL(DAC_SPE(IP,IS,ID))
             END DO
           END DO
         END DO
         ...
         DO IP = 1, MNP
           DO IS = 1, MSC
             DO ID = 1, MDC   
               AC2(IP,IS,ID) =  AC1(IP,IS,ID) + REAL(DAC_SOU(IP,IS,ID))
             END DO
           END DO
         END DO
