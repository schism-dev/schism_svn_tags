...
!#endif
  ...
  integer,save :: nz_r,ieqstate,kr_co, &
                  &ihconsv,isconsv,ihdif,mntr,ntracers2, & !ntracers2=ntracers+2
                  &ihydraulics,irouse_test,iwbl_itmax,nettype,nfltype, &
                  &ntetype,nsatype,ntrtype,nettype2,nnode_et,nfltype2,nnode_fl, &
                  &ntetype2,nnode_te,nsatype2,nnode_sa,ntrtype2,nnode_tr,inu_tr, &
...
!    integer :: ndt           ! Number of backtracking sub-steps fro Euler tracking
  ...
  integer,save,allocatable :: i34(:)           !elem. type 
...
!  logical,save,allocatable :: lelbc(:)
  ...
  real(rkind),save,allocatable :: trobc(:),tobc(:),sobc(:),vobc1(:),vobc2(:),tamp(:), &
...
!#ifdef USE_TIMOR
  ...
  real(rkind),save,allocatable :: wsink(:,:,:) !wsink(ntracers,nvrt,npa); sink velocity>=0 [m/s]
  real(rkind),save,allocatable   :: rhomud(:,:,:) ! rhomud(ntracers,nvrt,npa): Mud floc particle density [kg/m3]
!#endif /*USE_TIMOR*/
