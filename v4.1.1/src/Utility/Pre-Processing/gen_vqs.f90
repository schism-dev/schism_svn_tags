...
!    ifort -Bstatic -O2 -CB -o gen_vqs gen_vqs.f90 ~/SELFE/svn/trunk/src/Utility/UtilLib/schism_geometry.f90
!    pgf90 -O2 -mcmodel=medium  -Bstatic -o gen_vqs gen_vqs.f90 ~/SELFE/svn/trunk/src/Utility/UtilLib/schism_geometry.f90
      ...
      allocatable :: xybp(:,:),imap(:),transect_len(:),sigma_vqs(:,:)
...
!     Read in hgrid
      ...
      read(14,*)
      read(14,*)ne,np
      allocate(xnd(np),ynd(np),dp(np),elnode(3,ne),kbp0(np),kbp(np),eta2(np), &
     &sigma_vqs(nvrt,np),znd(nvrt,np),m0(np),z1tmp(nvrt),z2tmp(nvrt))
      ...
      print*, 'ns=',ns
      allocate(ic3(3,ne),elside(3,ne),isdel(2,ns),isidenode(2,ns),xcj(ns,2),ycj(ns,2))
      call schism_geometry(np,ne,ns,xnd,ynd,elnode,ic3,elside,isdel,isidenode,xcj,ycj)
      !deallocate()
