...
!											*
!       pgf90 -O2 -mcmodel=medium -Mbounds -Bstatic -o read_output7b_xyzt read_output7b_xyzt.f90 ~/SELFE/svn/trunk/src/Utility/UtilLib/compute_zcor.f90 
!****************************************************************************************
!
      program read_output_xyzt
      character*30  :: file    !< base file name (e.g. elev.61) of files to be read 
      character*80  :: vgrid
      character*80  :: stationfile  !< input file, e.g. station.xyzt
      character*80  :: outfile  !< output file name
      real(kind=4)  :: fill    !< fill value for na values
      call read_output7b_xyzt_input(file,vgrid,stationfile,outfile,fill)
      call read_output7b_xyzt(file,vgrid,stationfile,outfile,fill)
      end program


      subroutine read_output7b_xyzt_input(file,vgrid,stationfile,outfile,fill)
      use argparse
      implicit none
      character*30  :: file    !< base file name (e.g. elev.61) of files to be read 
      character(LEN=*)  :: vgrid
      ...
      real(kind=4)  :: fill    !< fill value for na values
      integer       :: comcount
      cmd_name  = "read_output7b_xyzt"
      call cla_init(cmd_name)
      call cla_register('-f','--file', 'base input file (e.g. salt.63) without block number prefex', cla_char,'')
      call cla_register('-i','--in',   'input file with options for this utility', cla_char,'')
      call cla_register('-v','--vgrid', 'vertical grid file (typically vgrid.in or ../vgrid.in', cla_char,'vgrid.in')
      call cla_register('-o','--out', 'name of output file', cla_char,'extract.out')
      call cla_register('-s','--stations', 'input file containing extraction points. First line is comment, second line says how many points, other lines are id x y z t', cla_char,'station.xyzt')
      call cla_register('-x','--fill', 'fill value for invalid data', cla_float,'-999999.')
      call cla_validate

      comcount = command_argument_count()
      if (comcount == 0) then
          infile = "read_output7b_xyzt.in"
      else
...
!      print*, 'Input values to be used for invalid place:'
      ...
      subroutine read_output7b_xyzt(file63,vgrid,stationfile,outfile,fill)
      parameter(nbyte=4)
...
!     Read in vgrid.in for 3D model only
...
!       Interpolate in time
        ...
        if(i23d==2) then
          ...
          write(18,'(e16.8,8(1x,f15.3))')t00(i)/86400.0,out4(1:ivs),x00(i),y00(i)
        else !3D
          if(iep(i)==0) then !no parents
            out4(1:ivs)=rjunk
          else
          ...
          write(18,'(e16.8,4(1x,f15.3))')t00(i)/86400.,out4(1),x00(i),y00(i),z00(i)
          if(ivs==2) write(19,'(e16.8,1x,f15.3)')t00(i)/86400.,out4(2)
        endif
      enddo !i=1,nxy
      close(18)
      if (i23d/=2) then
