...
!--------------------------------------------------------------------!
      ...
      USE schism_glbl, ONLY: rkind,nvrt,nea,dfv,idry_e,kbe,elnode,uu2,vv2,ze
...
!---------------------------------------------------------------------
        ...
        ! * Consistency check
        !IF (isnan(tau_w(i)).EQV..TRUE.) THEN
...
!--------------------------------------------------------------------!
      ...
      USE schism_glbl, ONLY: rkind,nvrt,nea,elnode,dpe,eta2,idry_e,dfv,    &
...
!- Local variables --------------------------------------------------!
      ...
      REAL(rkind) :: hwat,hh,tau_c,tau_wash,Ts
...
!---------------------------------------------------------------------
        ...
        if(d50<=0.or.s<=1) call parallel_abort('sed_roughness: d50<=0') 
...
!---------------------------------------------------------------------
            IF(tau_c.GT.tau_cr) THEN ! Initiation of motion
...
!'
     ...
     &theta_w,theta_cr,psi,Hwr,Lwr,d50,hwat,tau_c,uorb(i),tp(i),tau_w
                    !call parallel_abort(errmsg)
                    z0bld = 0.0d0
                  else
