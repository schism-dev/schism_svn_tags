...
!#endif
      ...
      real(rkind),allocatable :: fluxes_vol(:,:),fluxes_vol_gb(:,:) !fluxes output between regions
...
!$OMP parallel workshare default(shared)
          !trnd_nu is junk outside nudging zone
...
!$OMP master
      ...
      !Fluxes
      !fluxes_vol(1:max_flreg,:): volume (or other) fluxes from region i to i-1, with i>=1
      !(i.e. excluding region -1). 2nd index is used to store different fluxes
      allocate(fluxes_vol(max_flreg,5),fluxes_vol_gb(max_flreg,5),stat=istat)
...
!'
        if(iflux_e(ie0) .ne. iflux_e(ie) .and. iabs(iflux_e(ie0) - iflux_e(ie)) .eq. 1) then
          ...
          do k=kbs(i),nvrt-1
            ...
            fluxes_vol(itmp1,1)=fluxes_vol(itmp1,1)+ftmp
#ifdef USE_ANALYSIS
            !Other fluxes
            if(ftmp>=0) then !positive flux
              fluxes_vol(itmp1,2)=fluxes_vol(itmp1,2)+ftmp
              !salt flux
              fluxes_vol(itmp1,4)=fluxes_vol(itmp1,4)+ftmp*tr_el(2,k+1,ie0) !upwind
            else
              fluxes_vol(itmp1,3)=fluxes_vol(itmp1,3)+ftmp
              fluxes_vol(itmp1,5)=fluxes_vol(itmp1,5)+ftmp*tr_el(2,k+1,ie)
            endif
#endif
          enddo !k
...
!$OMP master
      ...
      cwtmp=mpi_wtime()
#endif
      call mpi_reduce(fluxes_vol,fluxes_vol_gb,max_flreg*5,rtype,MPI_SUM,0,comm,ierr)
#ifdef INCLUDE_TIMING
      wtimer(11,2)=wtimer(11,2)+mpi_wtime()-cwtmp
#endif
      if(myrank==0) then
        write(9,'(f16.6,6000(1x,e14.4))')time/86400,fluxes_vol_gb(1:max_flreg,1)
#ifdef USE_ANALYSIS
        write(9,'(f16.6,6000(1x,e14.4))')time/86400,fluxes_vol_gb(1:max_flreg,2)
        write(9,'(f16.6,6000(1x,e14.4))')time/86400,fluxes_vol_gb(1:max_flreg,3)
        write(9,'(f16.6,6000(1x,e14.4))')time/86400,fluxes_vol_gb(1:max_flreg,4)
        write(9,'(f16.6,6000(1x,e14.4))')time/86400,fluxes_vol_gb(1:max_flreg,5)
#endif
