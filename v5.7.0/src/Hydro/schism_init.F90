...
!     Make sure the order of optional modules is same during actual outputs 
!     so that iof() are in order. If no on/off flags are used, outputting in
!     schism_step can be in any order.
...
!$OMP parallel do default(shared) private(i)
        ...
        do i=1,npa
          ...
          !fluxprc is net flux if impose_net_flux/=0
        enddo
!$OMP end parallel do
