...
subroutine ice_evp
  ...
  real(rkind) :: dtevp,t_evp_inv,det1,det2,tmp1,tmp2, &
 ...
 &swild2(nea),alow(4),bdia(4),rrhs(3,4)
...
!      endif
      ...
      delta_inv=1/max(delta_ice(i),delta_min)
      zeta(i)=pp0*0.5*delta_inv
    enddo !i=1,nea
    ...
    do i=1,np
      ...
      rr1=zeta_nd*t_evp_inv*(eps11_nd+eps22_nd-delta_nd) !RHS for 1st eq
      rr2=zeta_nd*t_evp_inv*(eps11_nd-eps22_nd)
      rr3=zeta_nd*t_evp_inv*eps12_nd
...
!Error
      ...
      gam1=a_ice_nd/mass*cdwat*rho0*umod
      rxp=-grav*deta_dx+(a_ice_nd*stress_atm_ice(1,i)+dsig_1)/mass
      ryp=-grav*deta_dy+(a_ice_nd*stress_atm_ice(2,i)+dsig_2)/mass
