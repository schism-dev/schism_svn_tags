...
!---------------------------------------------------------------------------
  ...
  allocate( NO3(nvrt),NH4(nvrt),SiO4(nvrt),S1(nvrt),S2(nvrt),Z1(nvrt),Z2(nvrt),&
          ...
          & salt(nvrt),SPM(nvrt,nea),bio(nvrt,ntrc),bio0(nvrt,ntrc),qcos(ntrc),sqcos(nvrt,ntrc),&
          & mS2(7,nvrt,nea),mDN(7,nvrt,nea),mZ1(7,nvrt,nea),mZ2(7,nvrt,nea),&
          & sS2(nvrt,nea),sDN(nvrt,nea),sZ1(nvrt,nea),sZ2(nvrt,nea),nstep(nvrt,nea),stat=istat) 
...
!---------------------------------------------------------------------------
  use schism_glbl, only : rkind,npa,nea,ne_global,np_global,ipgl,elnode,i34
  ...
  !local variables
  integer :: i,j,k,negb,npgb,nd,itmp,itmp1(1),itmp2(1,1)
