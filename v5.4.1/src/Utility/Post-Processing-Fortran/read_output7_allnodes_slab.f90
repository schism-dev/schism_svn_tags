...
!       Outputs: extract_slab.gr3 (.xyuv for vectors)
...
!****************************************************************************************
!     ifort -O2 -mcmodel=medium -assume byterecl -o read_output7_allnodes_slab read_output7_allnodes_slab.f90 ~/SCHISM/svn/trunk/src/Utility/UtilLib/compute_zcor.f90
!     pgf90 -O2 -mcmodel=medium -Mbounds -o read_output7_allnodes_slab read_output7_allnodes_slab.f90 ~/SCHISM/svn/trunk/src/Utility/UtilLib/compute_zcor.f90
      program read_out
      parameter(nbyte=4)
      character*30 file63
            ...
            do i=1,np
              if(idry(i)==1) then
                ...
                if(zout>=ztmp(nvrt,i)) then !above F.S.
                  ...
                  if(k0==0) then
                    write(*,*)'read_output7*: failed to find a vertical level:',it1,i,ifs,zout,ztmp(:,i)
!'
                    stop
                  endif
            ...
            write(65,*)'#Day,zout: ',time/86400,zout
            if(ivs==1) write(65,*)ne,np

            do i=1,np
              if(ivs==1) then
                write(65,*)i,x(i),y(i),out3(1,i)
              else
                write(65,*)x(i),y(i),out3(1:2,i),sqrt(out3(1,i)**2+out3(2,i)**2)
              endif
            enddo !i=1,np
