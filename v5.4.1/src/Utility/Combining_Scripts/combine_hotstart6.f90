...
! Usage: ./combine_hotstart6 -h for help
! Inputs:
...
!
!  PGI compiler:
!  pgf90 -O2 -mcmodel=medium -Mbounds  -Bstatic -o combine_hotstart6 ../UtilLib/argparse.F90 combine_hotstart6.f90
!  ifort -O2 -CB -Bstatic -assume byterecl -o combine_hotstart6 ../UtilLib/argparse.F90 combine_hotstart6.f90

! Revisions: v5 with tracer parts changed
!================================================================================
...
!-------------------------------------------------------------------------------
  ...
  allocatable :: iwild(:),iwild2(:,:),swild(:,:,:),wild2(:),wild3(:),wild4(:)
...
!-------------------------------------------------------------------------------
  ...
  !Debug
  print*, 'istep, nproc, ntracers:',istep, nproc, ntracers
...
!-------------------------------------------------------------------------------
  ...
  allocate(iwild(ne_global),swild(1+ntracers,nvrt,ne_global),stat=istat)
  if(istat/=0) stop 'Allocation error (7)'
  do irank=0,nproc-1
    fgb2=fgb
    write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
    !Reserve 8 bytes for all integers as well
    ihot_len=8*(9+(1+(1+ntracers)*nvrt)*ner(irank))
    open(36,file=fgb2(1:lfgb)//'_hotstart',access='direct',recl=ihot_len,status='old')
    read(36,rec=1)icm,ised2d,ised3d,iha,icosine,imarsh,time,it,ifile,(iwild(ielg(irank,i)),(swild(:,j,ielg(irank,i)),j=1,nvrt),i=1,ner(irank))
    close(36)
  ...
  do i=1,ne_global
    write(37) i,iwild(i),(swild(:,j,i),j=1,nvrt)
  enddo !i
  ...
  do i=1,ne_global
    do l=1,ntracers
      write(98,*)i,l,swild(1+l,:,i)
      do j=1,nvrt
        if(swild(l+1,j,i)<-40) then
          print*, 'Negative conc:',i,l,j,swild(l+1,j,i)
          stop
        endif
      enddo !j
    enddo !l
  ...
  tmp1=maxval(swild) 
  tmp2=minval(swild) 
  if(abs(tmp1)+abs(tmp2)>1.e6) then
    print*, 'Wrong elem. data:',tmp1,tmp2
    stop
  endif
  ...
  !Read/write sides
  allocate(iwild(ns_global),swild(2,nvrt,ns_global),wild2((1+ntracers)*nvrt),stat=istat)
  if(istat/=0) stop 'Allocation error (7)'
  do irank=0,nproc-1
    fgb2=fgb
    write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
    ihot_len=8*(9+(1+(1+ntracers)*nvrt)*ner(irank)+(1+2*nvrt)*nsr(irank))
    open(36,file=fgb2(1:lfgb)//'_hotstart',access='direct',recl=ihot_len,status='old')
    read(36,rec=1)icm,ised2d,ised3d,iha,icosine,imarsh,time,it,ifile,(itmp,wild2(:),i=1,ner(irank)), &
     &(iwild(islg(irank,i)),(swild(:,j,islg(irank,i)),j=1,nvrt),i=1,nsr(irank))
    close(36)
  enddo !irank

  do i=1,ns_global
!Error: remove 0.d0 and change format eventually
    write(37) i,iwild(i),(swild(:,j,i),0.d0,0.d0,j=1,nvrt) !idry_s(i),(su2(j,i),sv2(j,i),tsd(j,i),ssd(j,i),j=1,nvrt)
  enddo !i

  !Debug
!  write(98,*)'Side vel'
!  do i=1,ns_global
!    write(98,*)i,iwild(i),swild(1,:,i)
!    write(98,*)i,iwild(i),swild(2,:,i)
!  enddo !i

  tmp1=maxval(swild) 
  tmp2=minval(swild) 
  if(abs(tmp1)+abs(tmp2)>1.e3) then
    print*, 'Wrong vel. data:',tmp1,tmp2
    stop
  endif
  ...
  !Read/write nodes
  allocate(iwild(np_global),swild(2*ntracers+7,nvrt,np_global),wild3(2*nvrt),wild4(np_global),stat=istat)
  if(istat/=0) stop 'Allocation error (7)'
  do irank=0,nproc-1
    fgb2=fgb
    write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
    ihot_len=8*(9+(1+(1+ntracers)*nvrt)*ner(irank)+(1+2*nvrt)*nsr(irank)+(2+(2*ntracers+7)*nvrt)*npr(irank))
    ...
    read(36,rec=1)icm,ised2d,ised3d,iha,icosine,imarsh,time,it,ifile,(itmp,wild2(:),i=1,ner(irank)), &
     ...
     &(wild4(iplg(irank,i)),iwild(iplg(irank,i)),(swild(:,j,iplg(irank,i)),j=1,nvrt),i=1,npr(irank))
    close(36)
  enddo !irank
  ...
  !  write(96,*)i,real(wild4(i))
    do j=1,nvrt
      do l=1,2*ntracers
        if(swild(l,j,i)<-40) then
          print*, 'Node conc<0:',i,j,l,swild(l,j,i)
          stop
        endif
      enddo !l
    enddo !j
  ...
  tmp1=maxval(swild) 
  tmp2=minval(swild) 
  if(abs(tmp1)+abs(tmp2)>1.e8) then
    print*, 'Wrong node data:',tmp1,tmp2
    stop
  endif
...
!          print*, 'i,j:',i,j,swild(1,j,ielg(irank,i))
  ...
  if(icosine==1) then
    allocate(swild(32,nvrt,ne_global),iwild2(nvrt,ne_global),stat=istat)
    if(istat/=0) stop 'Allocation error (13)' 
    do irank=0,nproc-1
      fgb2=fgb
      write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
      open(36,file=fgb2(1:lfgb)//'_hotstart',access='direct',recl=8,status='old')
      do i=1,ner(irank)
        do j=1,nvrt
          do k=1,7
            read(36,rec=ihotstp(irank)+1)swild(4*k-3,j,ielg(irank,i))
            read(36,rec=ihotstp(irank)+2)swild(4*k-2,j,ielg(irank,i))
            read(36,rec=ihotstp(irank)+3)swild(4*k-1,j,ielg(irank,i))
            read(36,rec=ihotstp(irank)+4)swild(4*k,j,ielg(irank,i))
            ihotstp(irank)=ihotstp(irank)+4
          enddo
          read(36,rec=ihotstp(irank)+1)swild(29,j,ielg(irank,i))
          read(36,rec=ihotstp(irank)+2)swild(30,j,ielg(irank,i))
          read(36,rec=ihotstp(irank)+3)swild(31,j,ielg(irank,i))
          read(36,rec=ihotstp(irank)+4)swild(32,j,ielg(irank,i))
          read(36,rec=ihotstp(irank)+5)iwild2(j,ielg(irank,i))
          ihotstp(irank)=ihotstp(irank)+5
        enddo
      enddo!i
      close(36)
    enddo!irank
    ...
    do i=1,ne_global
      write(37)i,(swild(1:32,j,i),iwild2(j,i),j=1,nvrt)
    enddo
    deallocate(swild,iwild2)
  endif!icosine=1
...
!      print*, 'B4 ICM:',tmp
      ...
      read(36,rec=ihotstp(irank)+3)nsed
      ihotstp(irank)=ihotstp(irank)+3
!      print*, 'SED3D dim=',MBEDP,Nbed,nsed,ne_global
      if(irank==0) then !allocate
        allocate(bed(Nbed,ne_global,MBEDP),rough_p(np_global), &
     &bed_frac(Nbed,ne_global,nsed),stat=istat)
        if(istat/=0) stop 'Allocation error (12)'
        !Test memory
        bed(Nbed,ne_global,MBEDP)=0 
        bed_frac(Nbed,ne_global,nsed)=0
      endif !irank  
...
!       print*, 'i,j:',i,j,wild2(iplg(irank,i))
    ...
    do i=1,ne_global    
      !bed(Nbed,nea,3), bed_frac(Nbed,nea,nsed)
  ...
  if(imarsh==1) then
    allocate(iwild(ne_global),stat=istat)
    if(istat/=0) stop 'Allocation error (9)'
    do irank=0,nproc-1
      fgb2=fgb
      write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
...
!      print*, 'B4 ICM:',tmp
      ...
      do i=1,ner(irank)
        read(36,rec=ihotstp(irank)+1)iwild(ielg(irank,i))
        ihotstp(irank)=ihotstp(irank)+1
      enddo !i
      close(36)
    enddo !irank
  ...
  !Deallocate
  deallocate(ner,npr,nsr,ihotstp,ielg,iplg,islg)
