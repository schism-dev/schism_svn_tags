...
struct => structures(7)
allocate(struct%substruct)
struct%substruct = structures(3)
...
! Compound weir and pipe, pipes closed, unsubmerged weir flow
upelev = 1.0d0
downelev = -1.75d0
structures(7)%height = huge(1.0)
structures(7)%substruct%op_down = 0.d0
structures(7)%substruct%op_up = 0.0
call calc_struc_flow(7,upelev,downelev,flow)
! width depth_flow coef sqrt(elev-crest) op_down
outside_calc =  10.d0*2.d0*7.5d-1*sqrt2grav*sqrt(2.d0)*0.5d0
SELFE_ASSERT_CLOSE(flow,outside_calc,epsilon)

!structures(7)%substruct%height=4.d0
structures(7)%substruct%width=2.d0
structures(7)%substruct%elev=-5.75d0
structures(7)%substruct%nduplicate=4
structures(7)%substruct%op_down=0.75d0
structures(7)%substruct%coef=0.5d0
! temporarily zero out the weir
structures(7)%coef = 0.d0
print *,pi*4.d0
outside_calc = pi*4.d0*0.75d0*0.5d0*4.d0*sqrt2grav*sqrt(2.75d0)
call calc_struc_flow(7,upelev,downelev,flow)
SELFE_ASSERT_CLOSE(flow,outside_calc,epsilon)
