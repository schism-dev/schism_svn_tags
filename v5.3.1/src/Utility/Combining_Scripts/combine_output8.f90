...
!         non-standard outputs!); fort.99 (hgrid.gr3 with all quads split)
!
!  ifort -Bstatic -O2 -assume byterecl -o combine_output8 combine_output8.f90 ../UtilLib/schism_geometry.f90 -Vaxlib -L/nasa/netcdf/3.6.0/intel/lib -lnetcdf -I/nasa/netcdf/3.6.0/intel/include

!  On TYPHOON:
!  pgf90 -O2 -mcmodel=medium  -Bstatic -o combine_output8 combine_output8.f90 -Mbounds ../UtilLib/schism_geometry.f90 -I/usr/local/amd64a/opteron/pgi/netcdf-3.6.2/include -L/usr/local/amd64a/opteron/pgi/netcdf-3.6.2/lib -lnetcdf
...
! Temp. fix: split all quads to get pure tri.
...
!  use netcdf
  ...
  character(len= 4) :: a_4
  integer,allocatable :: elnode(:,:),elnode2(:,:)
...
! Temp. fix (increased dim. related to ne_global)
  allocate(x(np_global),y(np_global),dp(np_global),kbp00(np_global),kbe(2*ne_global),i34(2*ne_global), &
          &np(0:nproc-1),ns(0:nproc-1),ne(0:nproc-1),elnode(4,2*ne_global),nm2(ne_global,4),eta2(np_global), &
          &ztot(nvrt),sigma(nvrt),cs(nvrt),outb(np_global,nvrt,2),ihot_len(0:nproc-1), &
          &outeb(2*ne_global,nvrt,2),dpe(2*ne_global),xctr(2*ne_global),yctr(2*ne_global), &
          &eta2e(2*ne_global),stat=istat)
...
! Temp. fixes for quads - split all quads without re-ordering nodes
  allocate(elnode2(3,2*ne_global))
  ne_new=ne_global
  do i=1,ne_global
    elnode2(1:3,i)=elnode(1:3,i)
    if(i34(i)==4) then  !append a new elem.
      ne_new=ne_new+1
      if(ne_new>2*ne_global) stop 'Too many elem.'
      elnode2(1,ne_new)=elnode(1,i)
      elnode2(2:3,ne_new)=elnode(3:4,i)
    endif
  enddo !i
  elnode(1:3,:)=elnode2(1:3,:)
  ne_global=ne_new
  i34=3

  write(99,*)'Split all quads'; write(99,*)ne_global,np_global
  do i=1,np_global
    write(99,'(i10,2(1x,e17.10),1x,f10.4)')i,x(i),y(i),dp(i)
  enddo !i
  do i=1,ne_global
    write(99,*)i,3,elnode(1:3,i)
  enddo !i
  close(99)
! End temp. fix
...
!     Write mode (header part only)
      ...
      iret=nf_put_vara_int(ncid,iele_id,data_start_2d,data_count_2d,elnode(1:3,1:ne_global))
      if(iret.ne.NF_NOERR) then
         print*, nf_strerror(iret); stop
      endif
