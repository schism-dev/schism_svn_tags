...
!-------------------------------------------------------------------------------
  ...
  do irank=0,nproc-1
    ...
    ihot_len=8*(7+(1+(1+ntracers)*nvrt)*ner(irank))
  ...
  do irank=0,nproc-1
    fgb2=fgb
    write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
    ihot_len=8*(7+(1+(1+ntracers)*nvrt)*ner(irank)+(1+2*nvrt)*nsr(irank))
    open(36,file=fgb2(1:lfgb)//'_hotstart',access='direct',recl=ihot_len,status='old')
...
!  enddo !i
  ...
  do irank=0,nproc-1
    fgb2=fgb
    write(fgb2(lfgb-3:lfgb),'(i4.4)') irank
    ihot_len=8*(7+(1+(1+ntracers)*nvrt)*ner(irank)+(1+2*nvrt)*nsr(irank)+(2+(2*ntracers+7)*nvrt)*npr(irank))
