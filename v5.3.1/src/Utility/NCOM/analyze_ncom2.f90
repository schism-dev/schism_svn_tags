! Read NRL/NCOM netcdf files and do analyses (vertical profiles).
!
! pturner 12-2004
!
...
!
! ifort -O3 -Bstatic -o analyze_ncom2 analyze_ncom2.f90 -Vaxlib  -I/usr/local/netcdf/include/ -L/usr/local/netcdf/lib -lnetcdf
!

! Typhoon:
! pgf90 -O2 -Bstatic -mcmodel=medium -o analyze_ncom2 analyze_ncom2.f90 -I/usr/local/amd64a/opteron/pgi/netcdf-3.6.2/include/ -L/usr/local/amd64a/opteron/pgi/netcdf-3.6.2/lib -lnetcdf

!   Input: 
!     (0) need to update ncomdir below;
!     (1) station.bp: x,y are lon., lat in degrees; z not used
!     (2) date.in: 1st line: year, month, day, starting CORIE day (0 if not sure), # of days; 2nd line: ifiletype (1: T,S; 2: UV).
...
!     integer :: i,j,k,i1,i2,i3,j1,j2,j3
      ...
      allocatable :: tempout(:,:), saltout(:,:), zout(:,:)
...
!     Extracting a subset of all lat,lon,h,vertical pos data
      if(ifile==0) then
        ...
        allocate( tempout(mnp,ilen))
        allocate( saltout(mnp,ilen))
        allocate( zout(mnp,ilen))
...
!               Find first wet pt
              ...
              do kk=1,kbp(in1,in2)-1
                wild2(1,1)=temp(ix,iy,kk)
                wild2(1,2)=salt(ix,iy,kk)
                wild2(2,1)=temp(ix+1,iy,kk)
                wild2(2,2)=salt(ix+1,iy,kk)
                wild2(3,1)=temp(ix+1,iy+1,kk)
                wild2(3,2)=salt(ix+1,iy+1,kk)
                wild2(4,1)=temp(ix,iy+1,kk)
                wild2(4,2)=salt(ix,iy+1,kk)
                tempout(i,kk)=wild2(1,1)*arco(1)+wild2(2,1)*arco(2)+wild2(3,1)*arco(3)+wild2(4,1)*arco(4)
                saltout(i,kk)=wild2(1,2)*arco(1)+wild2(2,2)*arco(2)+wild2(3,2)*arco(3)+wild2(4,2)*arco(4)
                zout(i,kk)=zm(ix,iy,kk)*arco(1)+zm(ix+1,iy,kk)*arco(2)+zm(ix+1,iy+1,kk)*arco(3)+zm(ix,iy+1,kk)*arco(4)
                if(abs(tempout(i,kk))>98.or.abs(saltout(i,kk))>98) then
                  write(11,*)'Failed to interpolate:',i,tempout(i,kk),saltout(i,kk)
                  stop
                endif 
              enddo !kk
            endif !rat<small1
...
!'
      ...
      do i=1,np
        do k=1,ilen
          if(zout(i,k)>-1.e10.and.saltout(i,k)>=0.and.tempout(i,k)>=0) then
            write(18,'(i6,2(1x,f12.3),1x,e13.6)')i,saltout(i,k),zout(i,k),timeout !S or u; time in PST
            write(19,'(i6,2(1x,f12.3),1x,e13.6)')i,tempout(i,k),zout(i,k),timeout !T or v
          endif
        enddo !k
      enddo !i
