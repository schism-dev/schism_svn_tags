...
!--------------------------------------------------------------------
  ...
  USE sed2d_mod, ONLY : Cdsed,cflsed,d50,d90,diffac,dpdxy,dpdxy_e,   &
                        ...
                        bed_delta,d50moy,d90moy,F_class,h_inf,       &
                        h_lim_max,h_lim_min,h_top,h2,nb_class,       &
                        nb_layer,morph_fac,ised_dump
  USE sed2d_friction
  USE sed2d_transport
  USE sed2d_filter
...
!- Wave parameters 
        ...
        wdirc = wdirc+(COS((270.d0-out_wwm(inode,9))*pi/180.d0))*ONETHIRD
        wdirs = wdirs+(SIN((270.d0-out_wwm(inode,9))*pi/180.d0))*ONETHIRD
#endif
...
!--------------------------------------------------------------------
      ...
      IF(IABS(idrag).GT.1) CALL compute_drag(d50moy_e,htot,z0_e(i),ue,ve,idrag,Cd_e(i))
...
!- Test if parameters are physically correct ------------------------
      ...
      IF(htot<0.d0 .OR. SQRT(ue*ue+ve*ve)>7.5d0 .OR. d50_e>0.05d0.OR.&
        ...
        &wlpeak > 1500.d0 .OR. Cd_e(i) > 10.d0.OR.z0_e(i) > 0.5d0) THEN
      ...
      SELECT CASE(itrans)
        ...
        CASE(3) !Soulsby - Van Rijn (Soulsby, 1997)
          CALL svr97_bedl(Cd_e(i),d50_e,ue,ve,htot,dpdxy_e(i,:),Ucr_c, &
                       &tau_cr,uorb,qb_e(i,:))
          CALL svr97_susp(Cd_e(i),d50_e,ue,ve,D_star,Ucr_c,uorb,qs_e(i,:))
...
!--------------------------------------------------------------------
    ...
    IF(imorpho == 1 .AND. it>iskip) THEN
      ...
      !Apply acceleration
      dh_class(:,k) = bed_delta(:)*morph_fac
    ENDIF
...
!YJZ: this check may be too restrictive?
          ...
          F_class(i,k,1)=(h_top*F_class(i,k,1)+sum_dh*F_class(i,k,2)-dh_class(i,k))/h_top
...
!YJZ: this check may be too restrictive?
          ...
          F_class(i,k,1)=((h_top+sum_dh)*F_class(i,k,1)-dh_class(i,k))/h_top

          IF(F_class(i,k,1).LT.0.d0) F_class(i,k,1)=0.d0

          !New sub-surface layer fractions
          F_class(i,k,2)=(h2(i)*F_class(i,k,2)-sum_dh*F_class(i,k,1))/(h2(i)-sum_dh) !>=0
        ENDIF !sum_dh sign
      ENDDO !k=1,nb_class
