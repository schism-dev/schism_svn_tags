...
!--------------------------------------------------------------------
  ...
  USE sed2d_mod, ONLY : d50,d90,diffac,h0_sed,iasym,idrag,idsed2d,   &
                        ...
                        nb_class,nb_layer,morph_fac,ised_dump
...
!- Bottom evolution -------------------------------------------------
  ...
  CALL get_param(fname,'MORPH_FAC',2,tmp_int,tmp_float,tmp_string)
  if(tmp_float<=0) CALL parallel_abort(msg//'MORPH_FAC')
  morph_fac=tmp_float
...
!- Display values in log file ---------------------------------------
  IF(myrank == 0) THEN
    ...
    IF (nb_class==1) THEN
      WRITE(idsed2d,*)'  D50       = ',d50(1)
      WRITE(idsed2d,*)'  D90       = ',d90(1)
    ELSE
    ...
    WRITE(idsed2d,*)'  morph_fac = ',morph_fac
