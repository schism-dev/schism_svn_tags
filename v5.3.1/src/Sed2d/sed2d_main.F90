...
!--------------------------------------------------------------------
  ...
  USE schism_glbl, ONLY : Cdp,dldxy,dp,dt,eta2,idry,idry_e,idry_s,ielg,   &
                        ipgl,indel,iplg,isdel,isidenode,isidenei2,elside,nea, &
                        nne,i34,elnode,np,npa,ns,nsa,pi,out_wwm,su2,sv2,     &
...
!JZ: lon/lat; hydraulics
    DO k = 1,ufilter
      DO i = 1,ns
        ...
        ENDDO
        su2_tmp2(i) = su2_tmp1(i)+0.125d0*(suru-4.d0*su2_tmp1(i))
        sv2_tmp2(i) = sv2_tmp1(i)+0.125d0*(surv-4.d0*sv2_tmp1(i))
      ENDDO !ns
      DO i = 1,ns
        if(isdel(2,i)==0.or.idry_s(i)==1) CYCLE
...
!--------------------------------------------------------------------
  ...
  USE schism_glbl, ONLY : Cdp,dldxy,dp,dt,eta2,idry,idry_s,iplg,isdel,       &
                        isidenode,islg,elnode,np,npa,ns,nsa,pi,out_wwm,  &
                        su2,sv2,rkind
  USE schism_msgp, ONLY : exchange_s2d,myrank,parallel_abort
