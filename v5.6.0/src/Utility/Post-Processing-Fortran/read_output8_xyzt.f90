...
!         for 3D variables (surface values for 2D variables) DEFINED @ nodes. Interpolation in time.
...
!											
! ifort -mcmodel=medium -CB -O2 -o read_output8_xyzt.WW ../UtilLib/extract_mod.f90 read_output8_xyzt.f90 ../UtilLib/compute_zcor.f90 ../UtilLib/pt_in_poly.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
!****************************************************************************************
!
      program read_out
      use netcdf
      ...
      integer,allocatable :: kbp(:),iday(:,:),irecord(:,:),node3(:,:),iep(:)
...
!...  Header
     ...
     &elnode(1:i34(ne),ne),nvrt,h0,x(np),y(np),dp(np),start_time
      ...
      allocate(out(3,nvrt,2),out2(2,nvrt,2),eta2(np),node3(nxy,3),arco(nxy,3), &
    &iep(nxy),iday(2,nxy),irecord(2,nxy),times(2,nxy),outvar(2,nvrt,np),stat=istat)
