...
!Routines & functions:
!SCRNNV: to be deleted
!sed_eq: solve mass-balance equations of 2 layers in sediment
!sed_zbrent: Brent's method to find SOD value
...
!    write(11,*)C2,C0d,j1,j2,pie1,pie2,m1,m2,s,KL,w,WS,k12,k2,H2,dt
    ...
    &C2,C0d,j1,j2,pie1,pie2,m1,m2,s,KL,w,WS,k12,k2,H2,dt
...
!  HS0=SED_COD(id)!in unit of g/m^3
!  SAL0=SED_SALT(id)
...
!    NH4T2TM1=NH4T2TM1-rat*tlfNH4sav(id)*dtw/HSED(id)
!    PO4T2TM1=PO4T2TM1-rat*tlfPO4sav(id)*dtw/HSED(id)
!    ROOTDO=-trtdosav(id)
!  endif !isav_icm

  !unit: g/m^3
  if(isav_icm==1.and.patchsav(id)==1)then
    NH4T2TM1=NH4T2TM1-tlfNH4sav(id)*dtw/HSED(id)
    PO4T2TM1=PO4T2TM1-tlfPO4sav(id)*dtw/HSED(id)
...
!  flxu=rat*WSUBNET(id)
  ...
  do i=1,3 !for 3 classes of POM
    ...
    enddo !j
  enddo !i
  !combination of PB1 and bottom Si
  ...
  !future: mapping flag
!check_ncai: unnecessary express of fraction
...
!  if(isav_icm==1.and.patchsav(id)==1) then
!    do i=1,3
!      flxpoc(id,i)=flxpoc(id,i)+rat*trtpocsav(id)*frcsav(i)
!      flxpon(id,i)=flxpon(id,i)+rat*trtponsav(id)*frnsav(i)
!      flxpop(id,i)=flxpop(id,i)+rat*trtpopsav(id)*frpsav(i)
!    enddo
!  endif
...
!------------------------------------------------------------------------
  ...
  W12=(VPMIX(id)*ZW12NOM(ind)/H2)*(POC1/1.0e2)*(1.0-KBENSTR*BENSTR)+DPMIN/H2

  !diffusion mixing velocity
  ...
  if(ierr==1) then
    ...
    call parallel_abort('sediment flux model: SOD (1)')!out of range
  elseif(ierr==2) then
  ...
  j2=ZHTASI(ind)*H2*CSISAT*PSI/(PSI+KMPSI)+flxs*SI0*rKSAp*SSI(id) !rKSAp ==0,future app with TSS
...
!  SED_BENSA(id)=JSI/rat
  ...
  NO31TM1S(id)  = NO31        !          NO3 in 1st layer
  HS1TM1S(id)   = HS1         !
  ...
  if(CNH4(id)<=0.or.CNO3(id)<0.or.CPIP(id)<0) then
    write(errmsg,*)'sed conc<0.0:',id,CNH4(id),CNO3(id),CPIP(id)
    call parallel_abort(errmsg)
  endif !sed conc
...
!------------------------------------------------------------------------------
  ...
  JO2NH4=AON*k12*NH41/s !unit: g/m^2 day
...
!  CSODHS=(ZHTAD1(ind)**2*fd1+ZHTAP1(ind)**2*fp1)*(O20/KMHSO2)*HST1/s
  ...
  C0d=HS0!unit: g/m^3
  j1=0.0
!  j2=max(AOC*XJC-AONO*JN2GAS,1.d-10)/rat!unit convert to g/m^2 day
  j2=max(AOC*XJC-AONO*JN2GAS,1.d-10) !unit: g/m^2 day
  k12=(fp1*ZHTAP1(ind)**2+fd1*ZHTAD1(ind)**2)*O20/KMHSO2
  k2=0.0
  call sed_eq(HS1,HS2,HST1,HST2,HST2TM1,pie1,pie2,m1,m2,s,KL12,W12,W2,H2,dtw,C0d,j1,j2,k12,k2)
  JHS=s*(HS1-HS0)
  !oxygen consumptiong
  CSODHS=k12*HS1/s 
...
!  j2=max(AOC*XJC-AONO*JN2GAS,1.d-10)/rat!unit convert to g/m^2 day
  ...
  !calculate CSOD
  !CSOD_ncai
  CSODCH4=k12*CH41/s!unit: g/m^2 day
  CSOD=CSODCH4+CSODHS
...
!  SFA(id)=area(id)
  ...
  !nan checked for water column tracers
