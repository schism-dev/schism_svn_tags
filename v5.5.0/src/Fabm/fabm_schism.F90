...
module fabm_schism
  ...
  use schism_glbl, only: ntracers,nvrt,tr_el,tr_nd,erho,idry_e,nea,npa,ne,np,bdy_frc,flx_sf,flx_bt,dt,elnode,i34,srad,windx,windy,ze,kbe,wsett,ielg,iplg,xnd,ynd,rkind,xlon,ylat,lreadll,iwsett
  use schism_msgp
  use fabm
  use fabm_types
  ...
  ! build model tree
  fs%model => fabm_create_model_from_file(namlst_unit,'fabm.nml')
  ...
  ! wind speed, surface radiation, bottom stress...
...
!write(0,*) 'fabm: done'
  ...
  if (status /= nf90_noerr) then
    write(0,*) '  read from file skipped, file not found: ',trim(ncfilename)
    return
  end if
