...
! Usage: ./combine_output10 -h for help
! Inputs:
...
!  ifort -cpp -O2 -assume byterecl -o combine_output10.XX ../UtilLib/argparse.F90 combine_output10.f90 ../UtilLib/schism_geometry.f90 -I$NETCDF/include -I$NETCDF_FORTRAN/include -L$NETCDF_FORTRAN/lib -L$NETCDF/lib -lnetcdf -lnetcdff
...
subroutine combine_output10(ibgn,iend,iwetdry)
!-------------------------------------------------------------------------------
  use netcdf
...
!  character(len=30),dimension(nfile)   :: files  !< base names (e.g. elev.61) of files to combine
 ...
 &ic3(:,:),isidenode(:,:),i23d(:),ivs(:),ndims(:),iu_id(:)
  real,allocatable :: ztot(:),sigma(:),cs(:),outb(:,:,:),eta2(:),outeb(:,:,:), &
...
! Read local_to_global_0000 for global info
  ...
  do irank=0,nproc-1
    ...
    read(10,*)np(irank),ne(irank),(x(iplg(irank,m)),y(iplg(irank,m)),dp(iplg(irank,m)),itmp,m=1,np(irank)), &
...
!    enddo !m
  ...
  &worka(2,nvrt,ns2),outb(2,nvrt,ns2),stat=istat)
...
!   Some compiles do not like 3 arguemnts in OR()
    iret = nf90_create(trim(fname), OR(NF90_CLOBBER,NF90_NETCDF4), ncid)
    !iret = nf90_create(trim(fname), OR(NF90_CLOBBER,NF90_NETCDF4,NF90_CLASSIC_MODEL), ncid)
    !define dimensions
...
20  FORMAT('seconds since ',I0.4,'-',I0.2,'-',I0.2,' ',A38)
    ...
    iret=nf90_put_att(ncid,itime_id,'units',cbuffer) ! len(trim(cbuffer)),cbuffer)
...
!    tempint_array(1)=2
    ...
    iret=nf90_def_var(ncid,'Mesh3D',NF90_INT,one_dim,i3Dgrid_id) !0,dummy_dim,i3Dgrid_id)
...
!    int_buffer(1) = 1  ! fortran indexing starts at 1
    ...
    iret=nf90_put_att(ncid,ix_id,'units','m or degrees')
    ...
    iret=nf90_put_att(ncid,iy_id,'units','m or degrees')
...
!    iret=nf90_put_att(ncid,inode_bottom_id,'start_index',1) !int_buffer)
    ...
    iret=nf90_put_att(ncid,ix_face_id,'standard_name',lat_standard_name)
...
!    iret=nf90_put_var(ncid,inode_bottom_id,kbp00,data_start_1d,data_count_1d) !(/1/),(/np_global/))
!    data_count_1d(1)=ns_global
!    iret=nf90_put_var(ncid,iedge_bottom_id,kbs,data_start_1d,data_count_1d) !,(/1/),(/ns_global/))
!    data_count_1d(1)=ne_global
!    iret=nf90_put_var(ncid,iele_bottom_id,kbe,data_start_1d,data_count_1d) !(/1/),(/ne_global/))
      ...
      do m=1,nvars
        ...
        do irank=0,nproc-1
          ...
          if(i23d(m)==0) then !time
            ...
            iret=nf90_get_var(ncid2,m,hc_array,data_start_1d,data_count_1d) 
            time=hc_array(1)
            !write(99,*)'time=',time
            ...
            if(ivs(m)==1) then
              ...
              data_count_2d(2)=1
              iret=nf90_get_var(ncid2,m,worka(1,1,1:data_count_2d(1)),data_start_2d,data_count_2d)
            else !vector
              data_start_3d(1:2)=1; data_start_3d(3)=ispool 
              data_count_3d(1)=2; data_count_3d(3)=1
              iret=nf90_get_var(ncid2,m,worka(1:2,1,1:data_count_3d(2)),data_start_3d,data_count_3d)
            endif !ivs
          else !3D
            if(ivs(m)==1) then
              data_start_3d(1:2)=1; data_start_3d(3)=ispool
              data_count_3d(1)=nvrt; data_count_3d(3)=1
              iret=nf90_get_var(ncid2,m,worka(1,:,1:data_count_3d(2)),data_start_3d,data_count_3d)
            else !vector
              data_start_4d(1:3)=1; data_start_4d(4)=ispool
              data_count_4d(1)=2; data_count_4d(2)=nvrt; data_count_4d(4)=1
              iret=nf90_get_var(ncid2,m,worka(1:2,:,1:data_count_4d(3)),data_start_4d,data_count_4d)
            endif !ivs
          endif !i23d
          iret=nf90_close(ncid2)
          ...
          if(i23d(m)<=3) then !node
            do i=1,np(irank)
              nd=iplg(irank,i)
              outb(1:2,:,nd)=worka(1:2,:,i)
!              if(i23d(m)/=1) outb(1:2,1:kbp00(nd)-1,nd)=NF90_FILL_FLOAT !fill 3D
            enddo !i
          else if(i23d(m)<=6) then
            do i=1,ne(irank)
              ie=ielg(irank,i)
              outb(1:2,:,ie)=worka(1:2,:,i)
!              if(i23d(m)/=4) outb(1:2,1:kbe(ie)-1,ie)=NF90_FILL_FLOAT
            enddo !i
          else
            do i=1,ns(irank)
              isd=islg(irank,i)
              outb(1:2,:,isd)=worka(1:2,:,i)
!              if(i23d(m)/=7) outb(1:2,1:kbs(isd)-1,isd)=NF90_FILL_FLOAT
            enddo !i
          endif !i23d
        enddo !irank
        ...
        if(mod(i23d(m)-1,3)/=0) then !3D
          if(i23d(m)<=3) then !node
            do i=1,np_global
              if(iwetdry/=0.or.kbp00(i)/=nvrt+1) outb(1:2,1:kbp00(i)-1,i)=NF90_FILL_FLOAT
            enddo !i
          else if(i23d(m)<=6) then
            do i=1,ne_global
              if(iwetdry/=0.or.kbe(i)/=nvrt+1) outb(1:2,1:kbe(i)-1,i)=NF90_FILL_FLOAT
            enddo !i
          else
            do i=1,ns_global
              if(iwetdry/=0.or.kbs(i)/=nvrt+1) outb(1:2,1:kbs(i)-1,i)=NF90_FILL_FLOAT
            enddo !i
          endif !i23d
        endif !3D
...
!              write(98,*)i,outb(1:ivs(m),1,i)
!            else
!              write(98,*)i,outb(1:ivs(m),nvrt,i)
!            endif
!          enddo !i
!        endif !i23d
        ...
        if(i23d(m)==0) then !time
          ...
          if(ivs(m)==1) then
            data_start_2d(1)=1; data_start_2d(2)=ispool
            data_count_2d(2)=1 
            iret=nf90_put_var(ncid,iu_id(m),outb(1,1,1:data_count_2d(1)),data_start_2d,data_count_2d)
          else
            data_start_3d(1:2)=1; data_start_3d(3)=ispool
            data_count_3d(1)=2; data_count_3d(3)=1
            iret=nf90_put_var(ncid,iu_id(m),outb(1:2,1,1:data_count_3d(2)),data_start_3d,data_count_3d)
          endif !ivs
        else !3D
          if(ivs(m)==1) then
            data_start_3d(1:2)=1; data_start_3d(3)=ispool
            data_count_3d(1)=nvrt; data_count_3d(3)=1
            iret=nf90_put_var(ncid,iu_id(m),outb(1,:,1:data_count_3d(2)),data_start_3d,data_count_3d)
          else
            data_start_4d(1:3)=1; data_start_4d(4)=ispool
            data_count_4d(1)=2; data_count_4d(2)=nvrt; data_count_4d(4)=1
            iret=nf90_put_var(ncid,iu_id(m),outb(1:2,:,1:data_count_4d(3)),data_start_4d,data_count_4d)
          endif
        endif !i23d
      enddo !m=1,nvars
    enddo !ispool=1,nrec
  ...
  deallocate(x,y,dp,kbp00,kbe,np,ns,ne,elnode,nm2,ztot,sigma,cs,outb,ihot_len, &
  &dpe,xctr,yctr,iplg,ielg,islg,ic3,elside,isdel,isidenode, &
  &xcj,ycj,dps,kbs)
  
end subroutine combine_output10

subroutine combine_output10_input(ibgn,iend,iwetdry)
use argparse
implicit none
...
if(comcount == 0) then
  stop './"combine_output10" -h for help'
...
integer                              :: inetcdf !< netcdf flag
call combine_output10_input(ibgn,iend,iwetdry)
call combine_output10(ibgn,iend,iwetdry)
end program
