...
!---------------------------------------------------------------------------------
  use schism_glbl, only : rkind,errmsg,NDTWQ,dt,tr_el,i34,elside,nea,nvrt,irange_tr,ntrs,idry_e, &
                        & isdel,kbs,zs,su2,sv2,npa,nne,elnode
  use schism_msgp, only : myrank,parallel_abort
  use icm_mod, only : iSed,iRea,iPh,PH_el,PH_nd,nspool_icm
  implicit none
...
!--------------------------------------------------------------------------------
  use schism_glbl, only : rkind,errmsg,tr_el,nvrt,irange_tr,ntrs,ze,kbe 
  ...
  elseif(imode==2) then
    do k=kbe(id)+1,nvrt
      ...
      do i=1,(21+4*iPh)
        if(.not.(tr_el(i-1+irange_tr(1,7),k,id)>0.or.tr_el(i-1+irange_tr(1,7),k,id)<=0)) then
          write(errmsg,*)'nan found in ICM(2) : ',tr_el(i-1+irange_tr(1,7),k,id),id,i,k
          call parallel_abort(errmsg)
        endif
      enddo
...
!----------------------------------------------------------------------------
  ...
  !ncai: sav
  real(kind=rkind) :: pmaxsav,fisav,fnsav,fpsav !growhth
  ...
  real(kind=rkind) :: tmp0,tmp,xtsav,zt0,hdep
...
!    enddo
    ...
    !hcansav(id)=min(hcansav(id),tdep)!limited in read_icm and calkwq
  ...
  if(hour>TU.and.hour<TD) then
    ...
    sLight0=rIa*sin(pi*(hour-TU)/Daylen)
    sLight=sLight0
...
!new22: debug
        ...
        if(zlfsav(k+1)>=0.and.zstsav(k+1)>=0) then !below canopy
          if (k==kcnpy) then
            ...
            tmp=hcansav(id)+ze(kbe(id),id)-zt0 !ze(klev,id)
            rKeh1=rKe0*tmp!accumulation for layer k, half
          ...
          iwcsav=iatcnpysav*(1-exp(-tmp))/tmp
     ...
     &iatcnpysav,ztcsav,tdep,hcansav(id)
          ...
          fisav=1
        endif !zlfsav(k)>0.and.zstsav(k)>0
...
!new23  !xcai
        write(97,*)'hcansavori and hcansav is:',hcansavori(id),hcansav(id)
        write(97,*)'sav growth rate at id for layer :',id,k,plfsav(k),pmaxsav,pmbssav,toptsav,xtsav,ktg1sav
      endif !isav_icm
!new23
      write(96,*)'hcansavori and hcansav is:',hcansavori(id),hcansav(id)
      write(96,*)'sav growth rate at id for layer:',id,k,plfsav(k),pmaxsav!,fisav,fnsav,fpsav
...
!----------------------------------------------------------------------------
  use icm_mod
  use schism_glbl, only : rkind,NDTWQ,nvrt,ielg,dt,ne,nvrt,ze,kbe
  use schism_msgp, only : myrank, parallel_abort
...
!new23
      if (id==136.and.k==7) then
        write(98,*)it,k,a,plfsav(k),bmlfsav(k),famsav,fplfsav,dtw
      endif
...
!new23!xcai
      ...
      if (id==136.and.k==7) then
        ...
        write(99,*)it,k,lfsav(k,id)
        write(99,*)it,k,stsav(k,id)
        write(99,*)it,k,rtsav(k,id)
      endif!id,k
      !write(99,*)'hcansavori and hcansav is:',hcansavori(id),hcansav(id)
    endif !isav_icm
    ...
    if(iZoo==1) then
      ...
      do i=1,2 !ZB1,ZB2
        ...
        if(xT>0.0) then
          ...
          ZBG(:,i)=ZBG(:,i)*exp(-rKTGZ2(i)*rtmp*rtmp)/sum1
        endif !rtmp
        ZBG0(:,i)=ZBG(:,i)
...
!    !ncai
!    if(isav_icm==1.and.patchsav(id)==1.and.(k)<hcansav(id)) then
...
!      fnsedsav=CNH4(id)/(CNH4(id)+(NH4(k,1)+NO3(k,1))*khnssav/khnwsav)
...
!      fpsedsav=CPIP(id)/(CPIP(id)+PO4t(k,1)*khpssav/khpwsav)
!
...
!    endif
    ...
    if(isav_icm==1.and.patchsav(id)==1) then
      ...
      !uptake of nutrient by lf phytosynthesis for this dt for each layer
!new22: debug
      fnsedsav=1; fpsedsav=1
...
!--------------------------------------------------------------------------------------
  ...
  if (isav_icm==1.and.patchsav(id)==1) then
    ...
    hcansav(id)=min(hcansavori(id),tdep)
